...
ARG GIT_REPO=https://github.com/aptos-labs/pont-core.git
ARG GIT_REF

RUN git clone $GIT_REPO ./ && git reset $GIT_REF --hard
RUN --mount=type=cache,target=/pont/target --mount=type=cache,target=$CARGO_HOME/registry \
  cargo build --release \
  -p pont-rosetta \
  && mkdir dist \
  && cp target/release/pont-rosetta dist/pont-rosetta

### Create image with pont-node and pont-rosetta ###
...
COPY --from=builder /pont/dist/pont-rosetta /usr/local/bin/pont-rosetta
...
WORKDIR /opt/pont/data

ENTRYPOINT ["/usr/local/bin/pont-rosetta"]
CMD ["online", "--config /opt/pont/fullnode.yaml"]
