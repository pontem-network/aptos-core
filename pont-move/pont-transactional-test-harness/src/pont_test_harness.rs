...
use pont_api_types::AsConverter;
use pont_crypto::{
    ed25519::{Ed25519PrivateKey, Ed25519PublicKey},
    hash::HashValue,
    ValidCryptoMaterialStringExt,
};
use pont_gas::{InitialGasSchedule, TransactionGasParameters};
use pont_state_view::StateView;
use pont_types::{
    access_path::AccessPath,
    account_config::{pont_test_root_address, AccountResource, CoinStoreResource},
...
/// The Pont transaction test adapter.
///
...
///   - It executes transactions through PontVM, instead of MoveVM directly
struct PontTestAdapter<'a> {
...
/// Pont-specific arguments for the publish command.
#[derive(StructOpt, Debug)]
struct PontPublishArgs {
...
/// Pont-specifc arguments for the run command.
#[derive(StructOpt, Debug)]
struct PontRunArgs {
    #[structopt(long = "private-key", parse(try_from_str = RawPrivateKey::parse))]
    private_key: Option<RawPrivateKey>,
...
/// Pont-specifc arguments for the init command.
#[derive(StructOpt, Debug)]
struct PontInitArgs {
...
/// Default private key mappings for special Pont accounts.
fn pont_framework_private_key_mapping() -> Vec<(String, Ed25519PrivateKey)> {
...
static PRECOMPILED_APTOS_FRAMEWORK: Lazy<FullyCompiledProgram> = Lazy::new(|| {
    ...
    match program_res {
        Ok(af) => af,
        Err((files, errors)) => {
            eprintln!("!!!Pont Framework failed to compile!!!");
...
impl<'a> PontTestAdapter<'a> {
    ...
    /// Obtain the PontCoin amount under address `signer_addr`
    fn fetch_account_balance(&self, signer_addr: &AccountAddress) -> Result<u64> {
        let pont_coin_tag = CoinStoreResource::struct_tag();

        let coin_access_path =
            AccessPath::resource_access_path(ResourceKey::new(*signer_addr, pont_coin_tag.clone()));
        ...
        let annotated =
            MoveValueAnnotator::new(&self.storage).view_resource(&pont_coin_tag, &balance_blob)?;
    ...
    fn run_transaction(&mut self, txn: Transaction) -> Result<TransactionOutput> {
        let mut outputs = PontVM::execute_block_and_keep_vm_status(vec![txn], &self.storage)?;
    ...
    fn create_and_fund_account(&mut self, account_addr: AccountAddress, amount: u64) {
        let parameters = self
            .fetch_transaction_parameters(&pont_test_root_address(), None, None, None, None)
            .unwrap();

        let txn = RawTransaction::new(
            pont_test_root_address(),
            parameters.sequence_number,
            cached_packages::pont_stdlib::pont_account_create_account(account_addr),
        ...
        let txn = RawTransaction::new(
            ...
            cached_packages::pont_stdlib::pont_coin_mint(account_addr, amount),
            parameters.max_gas_amount,
            parameters.gas_unit_price,
            parameters.expiration_timestamp_secs,
            ChainId::test(),
        ...
        self.run_transaction(Transaction::UserTransaction(txn))
            .expect("Failed to mint pont coin. This should not happen.");
    }
}

impl<'a> MoveTestAdapter<'a> for PontTestAdapter<'a> {
    type ExtraInitArgs = PontInitArgs;
    type ExtraPublishArgs = PontPublishArgs;
    type ExtraRunArgs = PontRunArgs;
    type Subcommand = PontSubCommand;
    type ExtraValueArgs = ();
    ...
    fn init(
        ...
        for (name, private_key) in pont_framework_private_key_mapping() {
    ...
    fn handle_subcommand(&mut self, input: TaskInput<Self::Subcommand>) -> Result<Option<String>> {
        match input.command {
            PontSubCommand::BlockCommand(block_cmd) => {
            ...
            PontSubCommand::ViewTableCommand(view_table_cmd) => {
...
pub fn run_pont_test(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    // TODO: remove once bundles removed
    pont_vm::pont_vm::allow_module_bundle_for_test();
    run_test_impl::<PontTestAdapter>(path, Some(&*PRECOMPILED_APTOS_FRAMEWORK))
}
