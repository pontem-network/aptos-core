...
use pont_vm::{
    ...
    parallel_executor::ParallelPontVM,
    PontVM, VMExecutor, VMValidator,
};
...
///
/// This struct is a mock in-memory implementation of the Pont executor.
#[derive(Debug)]
...
impl FakeExecutor {
    ...
    fn set_tracing(&mut self, test_name: &str, file_name: String) {
        ...
        if let Some(env_trace_dir) = env::var_os(ENV_TRACE_DIR) {
            ...
            let mut name_file = OpenOptions::new()
                ...
                .unwrap();
            write!(name_file, "{}::{}", test_name, pont_version).unwrap();
            for sub_dir in &[
                TRACE_DIR_META,
                TRACE_DIR_DATA,
                TRACE_DIR_INPUT,
    ...
    pub fn execute_block_and_keep_vm_status(
        ...
        PontVM::execute_block_and_keep_vm_status(
            txn_block
                .into_iter()
                .map(Transaction::UserTransaction)
                .collect(),
    ...
    pub fn execute_transaction_block_parallel(
        ...
        let (result, _) = ParallelPontVM::execute_block(
            txn_block,
            &self.data_store,
    ...
    pub fn execute_transaction_block(
        ...
        let output = PontVM::execute_block(txn_block.clone(), &self.data_store);
    ...
    pub fn verify_transaction(&self, txn: SignedTransaction) -> VMValidatorResult {
        let vm = PontVM::new(self.get_state_view());
