...
//! Test infrastructure for modeling Pont accounts.
...
/// Details about a Pont account.
///
/// Tests will typically create a set of `Account` instances to run transactions on. This type
/// encodes the logic to operate on and verify operations on any Pont account.
...
impl Account {
    ...
    /// not automatically get added to the Pont store. To add an account to the store, use
    ...
    pub fn with_keypair(privkey: Ed25519PrivateKey, pubkey: Ed25519PublicKey) -> Self {
        let addr = pont_types::account_address::from_public_key(&pubkey);
        Account {
            addr,
            privkey,
            pubkey,
    ...
    /// Creates a new account representing the pont root account in memory.
    ///
    /// The address will be [`pont_test_root_address`][account_config::pont_test_root_address], and
    /// the account will use [`GENESIS_KEYPAIR`][struct@GENESIS_KEYPAIR] as its keypair.
    pub fn new_pont_root() -> Self {
        Self::new_genesis_account(account_config::pont_test_root_address())
    }
