...
use crate::gas_meter::{
    InitialGasSchedule, PontGasParameters, ToOnChainGasSchedule, LATEST_GAS_FEATURE_VERSION,
};
use anyhow::Result;
use clap::Parser;
...
fn generate_script(gas_schedule: &GasScheduleV2) -> Result<String> {
    ...
    emitln!(writer, "use pont_framework::pont_governance;");
    emitln!(writer, "use pont_framework::gas_schedule;");
    emitln!(writer);
    ...
    emitln!(
        writer,
        "let framework_signer = pont_governance::resolve(proposal_id, @{});\n",
        AccountAddress::ONE,
    );
...
fn pont_framework_path() -> PathBuf {
    Path::join(
        Path::new(env!("CARGO_MANIFEST_DIR")),
        "../framework/pont-framework",
    )
}
...
pub fn generate_update_proposal(args: &GenArgs) -> Result<()> {
    ...
    pack.add_local_dep("PontFramework", &pont_framework_path().to_string_lossy());
