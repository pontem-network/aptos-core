...
module shared_account::SharedAccount {
    ...
    public entry fun set_up(user: signer, test_user1: signer, test_user2: signer) : address acquires SharedAccountEvent {
        ...
        pont_framework::pont_account::create_account(user_addr1);
        pont_framework::pont_account::create_account(user_addr2);
    ...
    #[test(user = @0x1111, test_user1 = @0x1112, test_user2 = @0x1113, core_framework = @pont_framework)]
    public entry fun test_disperse(user: signer, test_user1: signer, test_user2: signer, core_framework: signer) acquires SharedAccount, SharedAccountEvent {
        ...
        let (burn_cap, mint_cap) = pont_coin::initialize_for_test(&core_framework);

        let shared_account = borrow_global<SharedAccount>(resource_addr);
        let resource_signer = account::create_signer_with_capability(&shared_account.signer_capability);
        coin::register<PontCoin>(&resource_signer);
        coin::deposit(resource_addr, coin::mint(1000, &mint_cap));
        disperse<PontCoin>(resource_addr);
        coin::destroy_mint_cap<PontCoin>(mint_cap);
        coin::destroy_burn_cap<PontCoin>(burn_cap);

        assert!(coin::balance<PontCoin>(user_addr1) == 200, 0);
        assert!(coin::balance<PontCoin>(user_addr2) == 800, 1);
    }
