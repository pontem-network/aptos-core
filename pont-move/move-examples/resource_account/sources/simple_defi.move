...
/// (2) when exchanging coins, we call `exchange_to` to swap `PontCoin` to `ChloesCoin`, and `exchange_from` to swap `PontCoin` from `ChloesCoin`
module resource_account::simple_defi {
    use std::signer;
    use std::string;

    use pont_framework::account;
    use pont_framework::coin::{Self, Coin, MintCapability, BurnCapability};
    ...
    /// Exchange PontCoin to ChloesCoin
    public fun exchange_to(a_coin: Coin<PontCoin>): Coin<ChloesCoin> acquires ModuleData {
    ...
    /// Exchange ChloesCoin to PontCoin
    public fun exchange_from(c_coin: Coin<ChloesCoin>): Coin<PontCoin> acquires ModuleData {
        ...
        coin::withdraw<PontCoin>(&resource_signer, amount)
    }
    ...
    public entry fun exchange_to_entry(account: &signer, amount: u64) acquires ModuleData {
        let a_coin = coin::withdraw<PontCoin>(account, amount);
    ...
    #[test(origin_account = @0xcafe, resource_account = @0x0b6beee9bc1ad3177403a04efeefb1901c12b7b575ac5124c0205efc0dd2e32a, framework = @pont_framework)]
    public entry fun test_exchange_to_and_exchange_from(origin_account: signer, resource_account: signer, framework: signer) acquires ModuleData {
        ...
        let (pont_coin_burn_cap, pont_coin_mint_cap) = pont_coin::initialize_for_test(&framework);

        // exchange from 5 pont coins to 5 chloe's coins & assert the results are expected
        let five_a_coins = coin::mint(5, &pont_coin_mint_cap);
        let c_coins = exchange_to(five_a_coins);
        assert!(coin::value(&c_coins) == 5, 0);
        assert!(coin::balance<PontCoin>(signer::address_of(&resource_account)) == 5, 1);
        assert!(coin::balance<ChloesCoin>(signer::address_of(&resource_account)) == 0, 2);

        // exchange from 5 chloe's coins to 5 pont coins & assert the results are expected
        let a_coins = exchange_from(c_coins);
        assert!(coin::value(&a_coins) == 5, 0);
        assert!(coin::balance<PontCoin>(signer::address_of(&resource_account)) == 0, 3);
        ...
        coin::burn(a_coins, &pont_coin_burn_cap);
        coin::destroy_mint_cap<PontCoin>(pont_coin_mint_cap);
        coin::destroy_burn_cap<PontCoin>(pont_coin_burn_cap);
    }
}
