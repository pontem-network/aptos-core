...
module mint_nft::minting {
    ...
    public entry fun test_happy_path(origin_account: signer, collection_token_minter: signer, nft_receiver: signer, nft_receiver2: signer, pont_framework: signer) acquires CollectionTokenMinter {
        ...
        set_up_test(origin_account, &collection_token_minter, &minter_pk, pont_framework, &nft_receiver, 10);
    ...
    public entry fun test_minting_expired(origin_account: signer, collection_token_minter: signer, nft_receiver: signer, pont_framework: signer) acquires CollectionTokenMinter {
        let (minter_sk, minter_pk) = ed25519::generate_keys();
        set_up_test(origin_account, &collection_token_minter, &minter_pk, pont_framework, &nft_receiver, 10000000);
        let receiver_addr = signer::address_of(&nft_receiver);
        let proof_challenge = MintProofChallenge {
            receiver_account_sequence_number: account::get_sequence_number(receiver_addr),
            receiver_account_address: receiver_addr,
