...
pub trait PontValidatorInterface: Sync {
    ...
    fn get_framework_modules_by_version(&self, version: Version) -> Result<Vec<CompiledModule>> {
        ...
        for module_bytes in self
            ...
            .ok_or_else(|| anyhow!("Failure reading pont root address state"))?
            .get_modules()
        {
            acc.push(
    ...
    /// 1. Pont Framework code address
    /// 2. All validator addresses
    fn get_admin_accounts(&self, version: Version) -> Result<Vec<(AccountAddress, AccountState)>> {
        let mut result = vec![];
        let pont_framework = self
            .get_account_state_by_version(CORE_CODE_ADDRESS, version)?
            .ok_or_else(|| anyhow!("Pont framework account doesn't exist"))?;

        // Get all validator accounts
        let validators = pont_framework
...
pub struct DebuggerStateView<'a> {
    db: &'a dyn PontValidatorInterface,
    version: Option<Version>,
}

impl<'a> DebuggerStateView<'a> {
    pub fn new(db: &'a dyn PontValidatorInterface, version: Option<Version>) -> Self {
        Self { db, version }
    }
