...
module pont_framework::timestamp {
    ...
    public(friend) fun set_time_has_started(pont_framework: &signer) {
        system_addresses::assert_pont_framework(pont_framework);
        let timer = CurrentTimeMicroseconds { microseconds: 0 };
        move_to(pont_framework, timer);
    }
    ...
    public fun update_global_time(
        ...
        // Can only be invoked by PontVM signer.
        system_addresses::assert_vm(account);

        let global_timer = borrow_global_mut<CurrentTimeMicroseconds>(@pont_framework);
    ...
    public fun set_time_has_started_for_testing(account: &signer) {
        if (!exists<CurrentTimeMicroseconds>(@pont_framework)) {
    ...
    public fun now_microseconds(): u64 acquires CurrentTimeMicroseconds {
        borrow_global<CurrentTimeMicroseconds>(@pont_framework).microseconds
    }
