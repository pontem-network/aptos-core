spec pont_framework::stake {
    ...
    spec module {
        ...
        invariant [suspendable] exists<ValidatorSet>(@pont_framework) ==> validator_set_is_valid();
        // After genesis, `PontCoinCapabilities`, `ValidatorPerformance` and `ValidatorSet` exist.
        invariant [suspendable] chain_status::is_operating() ==> exists<PontCoinCapabilities>(@pont_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<ValidatorPerformance>(@pont_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<ValidatorSet>(@pont_framework);
    }

    // A desired invariant for the validator set.
    spec fun validator_set_is_valid(): bool {
        let validator_set = global<ValidatorSet>(@pont_framework);
    ...
    spec fun spec_validator_index_upper_bound(): u64 {
        len(global<ValidatorPerformance>(@pont_framework).validators)
    }
    ...
    spec schema ResourceRequirement {
        requires exists<PontCoinCapabilities>(@pont_framework);
        requires exists<ValidatorPerformance>(@pont_framework);
        requires exists<ValidatorSet>(@pont_framework);
        requires exists<StakingConfig>(@pont_framework);
        requires exists<timestamp::CurrentTimeMicroseconds>(@pont_framework);
    }
}
