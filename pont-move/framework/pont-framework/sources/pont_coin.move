...
module pont_framework::pont_coin {
    use std::string;
    use std::error;
    use std::signer;
    use std::vector;
    ...
    use pont_framework::coin::{Self, BurnCapability, MintCapability};
    ...
    public(friend) fun initialize(pont_framework: &signer): (BurnCapability<PontCoin>, MintCapability<PontCoin>) {
        system_addresses::assert_pont_framework(pont_framework);

        let (burn_cap, freeze_cap, mint_cap) = coin::initialize_with_parallelizable_supply<PontCoin>(
            pont_framework,
            string::utf8(b"Pont Coin"),
            string::utf8(b"APT"),
            8, /* decimals */
            true, /* monitor_supply */
        );

        // Pont framework needs mint cap to mint coins to initial validators. This will be revoked once the validators
        // have been initialized.
        move_to(pont_framework, MintCapStore { mint_cap });
    ...
    public(friend) fun destroy_mint_cap(pont_framework: &signer) acquires MintCapStore {
        system_addresses::assert_pont_framework(pont_framework);
        let MintCapStore { mint_cap } = move_from<MintCapStore>(@pont_framework);
    ...
    public(friend) fun configure_accounts_for_test(
        ...
        // Mint the core resource account PontCoin for gas so it can execute system transactions.
        coin::register<PontCoin>(core_resources);
        let coins = coin::mint<PontCoin>(
            18446744073709551615,
            &mint_cap,
        );
        coin::deposit<PontCoin>(signer::address_of(core_resources), coins);
    ...
    public entry fun mint(
        ...
        let coins_minted = coin::mint<PontCoin>(amount, mint_cap);
        coin::deposit<PontCoin>(dst_addr, coins_minted);
    }
    ...
    #[test_only]
    public fun initialize_for_test(pont_framework: &signer): (BurnCapability<PontCoin>, MintCapability<PontCoin>) {
        aggregator_factory::initialize_aggregator_factory_for_test(pont_framework);
