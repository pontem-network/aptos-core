 ...
 *
 */
module pont_framework::pont_governance {
    use std::error;
    use std::option;
    use std::signer;
    ...
    use pont_framework::account::{Self, SignerCapability, create_signer_with_capability};
    use pont_framework::coin;
    use pont_framework::event::{Self, EventHandle};
    use pont_framework::governance_proposal::{Self, GovernanceProposal};
    ...
    public fun store_signer_cap(
        ...
        system_addresses::assert_framework_reserved_address(pont_framework);

        if (!exists<GovernanceResponsbility>(@pont_framework)) {
            move_to(pont_framework, GovernanceResponsbility { signer_caps: simple_map::create<address, SignerCapability>() });
        };

        let signer_caps = &mut borrow_global_mut<GovernanceResponsbility>(@pont_framework).signer_caps;
    ...
    fun initialize(
        ...
        voting::register<GovernanceProposal>(pont_framework);
        move_to(pont_framework, GovernanceConfig {
            voting_duration_secs,
            min_voting_threshold,
            required_proposer_stake,
        });
        move_to(pont_framework, GovernanceEvents {
            create_proposal_events: account::new_event_handle<CreateProposalEvent>(pont_framework),
            update_config_events: account::new_event_handle<UpdateConfigEvent>(pont_framework),
            vote_events: account::new_event_handle<VoteEvent>(pont_framework),
        });
        move_to(pont_framework, VotingRecords {
            votes: table::new(),
        });
        move_to(pont_framework, ApprovedExecutionHashes {
    ...
    public fun update_governance_config(
        ...
        let governance_config = borrow_global_mut<GovernanceConfig>(@pont_framework);
        ...
        let events = borrow_global_mut<GovernanceEvents>(@pont_framework);
    ...
    public fun get_voting_duration_secs(): u64 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@pont_framework).voting_duration_secs
    }

    public fun get_min_voting_threshold(): u128 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@pont_framework).min_voting_threshold
    }

    public fun get_required_proposer_stake(): u64 acquires GovernanceConfig {
        borrow_global<GovernanceConfig>(@pont_framework).required_proposer_stake
    }
    ...
    public entry fun create_proposal(
        ...
        let governance_config = borrow_global<GovernanceConfig>(@pont_framework);
        ...
        let total_voting_token_supply = coin::supply<PontCoin>();
    ...
    public entry fun vote(
        ...
        let voting_records = borrow_global_mut<VotingRecords>(@pont_framework);
        ...
        let proposal_expiration = voting::get_proposal_expiration_secs<GovernanceProposal>(@pont_framework, proposal_id);
        assert!(
            stake::get_lockup_secs(stake_pool) >= proposal_expiration,
            error::invalid_argument(EINSUFFICIENT_STAKE_LOCKUP),
        );
        ...
        let proposal_state = voting::get_proposal_state<GovernanceProposal>(@pont_framework, proposal_id);
    ...
    public fun add_approved_script_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        let approved_hashes = borrow_global_mut<ApprovedExecutionHashes>(@pont_framework);
        ...
        let execution_hash = voting::get_execution_hash<GovernanceProposal>(@pont_framework, proposal_id);
    ...
    public fun resolve(proposal_id: u64, signer_address: address): signer acquires ApprovedExecutionHashes, GovernanceResponsbility {
        voting::resolve<GovernanceProposal>(@pont_framework, proposal_id);
    ...
    public fun remove_approved_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        assert!(
            voting::is_resolved<GovernanceProposal>(@pont_framework, proposal_id),
            error::invalid_argument(EPROPOSAL_NOT_RESOLVED_YET),
        );

        let approved_hashes = &mut borrow_global_mut<ApprovedExecutionHashes>(@pont_framework).hashes;
    ...
    public fun reconfigure(pont_framework: &signer) {
        system_addresses::assert_pont_framework(pont_framework);
    ...
    public fun get_signer_testnet_only(
        ...
        assert!(pont_coin::has_mint_capability(core_resources), error::unauthenticated(EUNAUTHORIZED));
        get_signer(signer_address)
    }
    ...
    fun get_signer(signer_address: address): signer acquires GovernanceResponsbility {
        let governance_responsibility = borrow_global<GovernanceResponsbility>(@pont_framework);
    ...
    #[test(pont_framework = @pont_framework, proposer = @0x123, yes_voter = @0x234, no_voter = @345)]
    public entry fun test_voting(
        ...
        setup_voting(&pont_framework, &proposer, &yes_voter, &no_voter);
        ...
        let proposal_state = voting::get_proposal_state<GovernanceProposal>(signer::address_of(&pont_framework), 0);
        assert!(proposal_state == PROPOSAL_STATE_SUCCEEDED, proposal_state);

        // Add approved script hash.
        add_approved_script_hash(0);
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@pont_framework).hashes;
        assert!(*simple_map::borrow(&approved_hashes, &0) == execution_hash, 0);

        // Resolve the proposal.
        let account = resolve(0, @pont_framework);
        assert!(signer::address_of(&account) == @pont_framework, 1);
        assert!(voting::is_resolved<GovernanceProposal>(@pont_framework, 0), 2);
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@pont_framework).hashes;
    ...
    public entry fun test_can_remove_approved_hash_if_executed_directly_via_voting(
        ...
        // Resolve the proposal.
        voting::resolve<GovernanceProposal>(@pont_framework, 0);
        assert!(voting::is_resolved<GovernanceProposal>(@pont_framework, 0), 0);
        remove_approved_hash(0);
        let approved_hashes = borrow_global<ApprovedExecutionHashes>(@pont_framework).hashes;
        assert!(!simple_map::contains_key(&approved_hashes, &0), 1);
    }

    #[test(pont_framework = @pont_framework, proposer = @0x123, voter_1 = @0x234, voter_2 = @345)]
    ...
    public entry fun test_cannot_double_vote(
        ...
        setup_voting(&pont_framework, &proposer, &voter_1, &voter_2);
    ...
    public fun setup_voting(
        ...
        staking_config::initialize_for_test(pont_framework, 0, 1000, 2000, true, 0, 1, 100);
        initialize(pont_framework, 10, 100, 1000);
        store_signer_cap(
            pont_framework,
            @pont_framework,
            account::create_test_signer_cap(@pont_framework),
        );
        ...
        stake::create_validator_set(pont_framework, active_validators);
    ...
    public entry fun test_update_governance_config(
        ...
        account::create_account_for_test(signer::address_of(&pont_framework));
        initialize(&pont_framework, 1, 2, 3);
        update_governance_config(&pont_framework, 10, 20, 30);

        let config = borrow_global<GovernanceConfig>(@pont_framework);
    ...
    public fun initialize_for_verification(
        ...
        initialize(pont_framework, min_voting_threshold, required_proposer_stake, voting_duration_secs);
    }
}
