module pont_framework::genesis {
    ...
    use pont_framework::aggregator_factory;
    use pont_framework::pont_coin::{Self, PontCoin};
    use pont_framework::pont_governance;
    use pont_framework::block;
    use pont_framework::chain_id;
    use pont_framework::chain_status;
    use pont_framework::coin;
    use pont_framework::consensus_config;
    use pont_framework::gas_schedule;
    use pont_framework::reconfiguration;
    use pont_framework::stake;
    use pont_framework::staking_contract;
    use pont_framework::staking_config;
    use pont_framework::state_storage;
    use pont_framework::storage_gas;
    use pont_framework::timestamp;
    use pont_framework::transaction_fee;
    use pont_framework::transaction_validation;
    use pont_framework::version;
    use pont_framework::vesting;
    ...
    fun initialize(
        ...
        // Initialize the pont framework account. This is the account where system resources and modules will be
        // deployed to. This will be entirely managed by on-chain governance and no entities have the key or privileges
        // to use this account.
        let (pont_framework_account, pont_framework_signer_cap) = account::create_framework_reserved_account(@pont_framework);
        // Initialize account configs on pont framework account.
        account::initialize(&pont_framework_account);
        ...
        pont_governance::store_signer_cap(&pont_framework_account, @pont_framework, pont_framework_signer_cap);

        // put reserved framework reserved accounts under pont governance
        let framework_reserved_addresses = vector<address>[@0x2, @0x3, @0x4, @0x5, @0x6, @0x7, @0x8, @0x9, @0xa];
        while (!vector::is_empty(&framework_reserved_addresses)) {
            let address = vector::pop_back<address>(&mut framework_reserved_addresses);
            let (pont_account, framework_signer_cap) = account::create_framework_reserved_account(address);
            pont_governance::store_signer_cap(&pont_account, address, framework_signer_cap);
        };

        consensus_config::initialize(&pont_framework_account, consensus_config);
        version::initialize(&pont_framework_account, initial_version);
        stake::initialize(&pont_framework_account);
        ...
        storage_gas::initialize(&pont_framework_account);
        gas_schedule::initialize(&pont_framework_account, gas_schedule);

        // Ensure we can create aggregators for supply, but not enable it for common use just yet.
        aggregator_factory::initialize_aggregator_factory(&pont_framework_account);
        coin::initialize_supply_config(&pont_framework_account);

        chain_id::initialize(&pont_framework_account, chain_id);
        reconfiguration::initialize(&pont_framework_account);
        block::initialize(&pont_framework_account, epoch_interval_microsecs);
        state_storage::initialize(&pont_framework_account);
        timestamp::set_time_has_started(&pont_framework_account);
    }

    /// Genesis step 2: Initialize Pont coin.
    fun initialize_pont_coin(pont_framework: &signer) {
        let (burn_cap, mint_cap) = pont_coin::initialize(pont_framework);
        // Give stake module MintCapability<PontCoin> so it can mint rewards.
        stake::store_pont_coin_mint_cap(pont_framework, mint_cap);
        // Give transaction_fee module BurnCapability<PontCoin> so it can burn gas.
        transaction_fee::store_pont_coin_burn_cap(pont_framework, burn_cap);
    }

    /// Only called for testnets and e2e tests.
    fun initialize_core_resources_and_pont_coin(
        ...
        pont_coin::configure_accounts_for_test(pont_framework, &core_resources, mint_cap);
    }

    fun create_accounts(pont_framework: &signer, accounts: vector<AccountMap>) {
        let i = 0;
    ...
    fun create_account(pont_framework: &signer, account_address: address, balance: u64): signer {
        if (account::exists_at(account_address)) {
            ...
            pont_coin::mint(pont_framework, account_address, balance);
            account
        }
    }
    ...
    fun create_employee_validators(
        ...
        while (i < num_employee_groups) {
            ...
            while (j < num_employees_in_group) {
                ...
                let total = coin::balance<PontCoin>(*account);
                let coins = coin::withdraw<PontCoin>(&employee, total);
    ...
    fun create_initialize_validators_with_commission(
        ...
        while (i < num_validators) {
            ...
            create_initialize_validator(pont_framework, validator, use_staking_contract);

            i = i + 1;
        };

        // Destroy the pont framework account's ability to mint coins now that we're done with setting up the initial
        // validators.
        pont_coin::destroy_mint_cap(pont_framework);
    ...
    fun create_initialize_validators(pont_framework: &signer, validators: vector<ValidatorConfiguration>) {
        let i = 0;
        let num_validators = vector::length(&validators);
        ...
        create_initialize_validators_with_commission(pont_framework, false, validators_with_commission);
    }
    ...
    fun create_initialize_validator(
        ...
        let owner = &create_account(pont_framework, validator.owner_address, validator.stake_amount);
        create_account(pont_framework, validator.operator_address, 0);
        create_account(pont_framework, validator.voter_address, 0);
    ...
    fun set_genesis_end(pont_framework: &signer) {
        chain_status::set_genesis_end(pont_framework);
    }
    ...
    fun initialize_for_verification(
        ...
        initialize_pont_coin(pont_framework);

        pont_governance::initialize_for_verification(
            pont_framework,
        ...
        create_initialize_validators(pont_framework, validators);
        set_genesis_end(pont_framework);
    }
    ...
    fun test_setup() {
        setup();
        assert!(account::exists_at(@pont_framework), 1);
    ...
    fun test_create_account(pont_framework: &signer) {
        setup();
        initialize_pont_coin(pont_framework);

        let addr = @0x121341; // 01 -> 0a are taken
        let test_signer_before = create_account(pont_framework, addr, 15);
        let test_signer_after = create_account(pont_framework, addr, 500);
        assert!(test_signer_before == test_signer_after, 0);
        assert!(coin::balance<PontCoin>(addr) == 15, 1);
    }

    #[test(pont_framework = @0x1)]
    fun test_create_accounts(pont_framework: &signer) {
        setup();
        initialize_pont_coin(pont_framework);
        ...
        create_accounts(pont_framework, accounts);
        assert!(coin::balance<PontCoin>(addr0) == 12345, 0);
        assert!(coin::balance<PontCoin>(addr1) == 67890, 1);

        create_account(pont_framework, addr0, 23456);
        assert!(coin::balance<PontCoin>(addr0) == 12345, 2);
    }
}
