...
module pont_framework::stake {
    ...
    struct StakePool has key {
        // active stake
        active: Coin<PontCoin>,
        // inactive stake, can be withdrawn
        inactive: Coin<PontCoin>,
        // pending activation for next epoch
        pending_active: Coin<PontCoin>,
        // pending deactivation for next epoch
        pending_inactive: Coin<PontCoin>,
    ...
    struct PontCoinCapabilities has key {
        mint_cap: MintCapability<PontCoin>,
    }
    ...
    public fun get_validator_state(pool_address: address): u64 acquires ValidatorSet {
        let validator_set = borrow_global<ValidatorSet>(@pont_framework);
    ...
    public fun get_current_epoch_proposal_counts(validator_index: u64): (u64, u64) acquires ValidatorPerformance {
        let validator_performances = &borrow_global<ValidatorPerformance>(@pont_framework).validators;
    ...
    public(friend) fun initialize(pont_framework: &signer) {
        ...
        move_to(pont_framework, ValidatorSet {
            consensus_scheme: 0,
        ...
        move_to(pont_framework, ValidatorPerformance {
    ...
    public(friend) fun store_pont_coin_mint_cap(pont_framework: &signer, mint_cap: MintCapability<PontCoin>) {
        system_addresses::assert_pont_framework(pont_framework);
        move_to(pont_framework, PontCoinCapabilities { mint_cap })
    }
    ...
    public fun remove_validators(
        ...
        let validator_set = borrow_global_mut<ValidatorSet>(@pont_framework);
    ...
    fun initialize_owner(owner: &signer) acquires AllowedValidators {
        ...
        move_to(owner, StakePool {
            active: coin::zero<PontCoin>(),
            pending_active: coin::zero<PontCoin>(),
            pending_inactive: coin::zero<PontCoin>(),
            inactive: coin::zero<PontCoin>(),
            locked_until_secs: 0,
    ...
    public entry fun add_stake(owner: &signer, amount: u64) acquires OwnerCapability, StakePool, ValidatorSet {
        ...
        assert_owner_cap_exists(owner_address);
        let ownership_cap = borrow_global<OwnerCapability>(owner_address);
        add_stake_with_cap(ownership_cap, coin::withdraw<PontCoin>(owner, amount));
    }

    /// Add `coins` into `pool_address`. this requires the corresponding `owner_cap` to be passed in.
    public fun add_stake_with_cap(owner_cap: &OwnerCapability, coins: Coin<PontCoin>) acquires StakePool, ValidatorSet {
        let pool_address = owner_cap.pool_address;
        assert_stake_pool_exists(pool_address);
        ...
        if (is_current_epoch_validator(pool_address)) {
            coin::merge<PontCoin>(&mut stake_pool.pending_active, coins);
        } else {
            coin::merge<PontCoin>(&mut stake_pool.active, coins);
        };
    ...
    public fun unlock_with_cap(amount: u64, owner_cap: &OwnerCapability) acquires StakePool {
        ...
        coin::merge<PontCoin>(&mut stake_pool.pending_inactive, unlocked_stake);
    ...
    public entry fun withdraw(
        ...
        coin::deposit<PontCoin>(owner_address, coins);
    }
    ...
    public fun withdraw_with_cap(
        ...
        withdraw_amount: u64
    ): Coin<PontCoin> acquires StakePool, ValidatorSet {
        let pool_address = owner_cap.pool_address;
        assert_stake_pool_exists(pool_address);
        let stake_pool = borrow_global_mut<StakePool>(pool_address);
        ...
        if (withdraw_amount == 0) return coin::zero<PontCoin>();
    ...
    public(friend) fun update_performance_statistics(proposer_index: Option<u64>, failed_proposer_indices: vector<u64>) acquires ValidatorPerformance {
        ...
        let validator_perf = borrow_global_mut<ValidatorPerformance>(@pont_framework);
    ...
    /// power.
    public(friend) fun on_new_epoch() acquires StakePool, PontCoinCapabilities, ValidatorConfig, ValidatorPerformance, ValidatorSet {
        let validator_set = borrow_global_mut<ValidatorSet>(@pont_framework);
        let config = staking_config::get();
        let validator_perf = borrow_global_mut<ValidatorPerformance>(@pont_framework);
    ...
    ) acquires StakePool, PontCoinCapabilities, ValidatorConfig {
        let stake_pool = borrow_global_mut<StakePool>(pool_address);
        let validator_config = borrow_global<ValidatorConfig>(pool_address);
    ...
    fun distribute_rewards(
        stake: &mut Coin<PontCoin>,
        num_successful_proposals: u64,
        num_total_proposals: u64,
        rewards_rate: u64,
        rewards_rate_denominator: u64,
    ): u64 acquires PontCoinCapabilities {
        ...
        if (rewards_amount > 0) {
            let mint_cap = &borrow_global<PontCoinCapabilities>(@pont_framework).mint_cap;
    ...
    public fun configure_allowed_validators(pont_framework: &signer, accounts: vector<address>) acquires AllowedValidators {
        let pont_framework_address = signer::address_of(pont_framework);
        system_addresses::assert_pont_framework(pont_framework);
        if (!exists<AllowedValidators>(pont_framework_address)) {
            move_to(pont_framework, AllowedValidators { accounts });
        } else {
            let allowed = borrow_global_mut<AllowedValidators>(pont_framework_address);
            allowed.accounts = accounts;
        }
    }

    fun is_allowed(account: address): bool acquires AllowedValidators {
        if (!exists<AllowedValidators>(@pont_framework)) {
            true
        } else {
            let allowed = borrow_global<AllowedValidators>(@pont_framework);
    ...
    #[test_only]
    public fun initialize_for_test(pont_framework: &signer) {
        initialize_for_test_custom(pont_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 1000000);
    }
    ...
    ) acquires PontCoinCapabilities, StakePool, ValidatorConfig, ValidatorPerformance, ValidatorSet {
    ...
    public fun initialize_for_test_custom(
        ...
        timestamp::set_time_has_started_for_testing(pont_framework);
        if (!exists<ValidatorSet>(@pont_framework)) {
            initialize(pont_framework);
        };
        ...
        if (!exists<PontCoinCapabilities>(@pont_framework)) {
            let (burn_cap, mint_cap) = pont_coin::initialize_for_test(pont_framework);
            store_pont_coin_mint_cap(pont_framework, mint_cap);
            coin::destroy_burn_cap<PontCoin>(burn_cap);
        };
    }

    // This function assumes the stake module already the capability to mint pont coins.
    #[test_only]
    public fun mint_coins(amount: u64): Coin<PontCoin> acquires PontCoinCapabilities {
        let mint_cap = &borrow_global<PontCoinCapabilities>(@pont_framework).mint_cap;
        coin::mint(amount, mint_cap)
    }

    #[test_only]
    public fun mint(account: &signer, amount: u64) acquires PontCoinCapabilities {
        let account_address = signer::address_of(account);
        if (!coin::is_account_registered<PontCoin>(account_address)) {
            coin::register<PontCoin>(account);
        };
    ...
    public fun mint_and_add_stake(
        account: &signer, amount: u64) acquires PontCoinCapabilities, OwnerCapability, StakePool, ValidatorSet {
        mint(account, amount);
    ...
    public fun initialize_test_validator(
        ...
        should_end_epoch: bool,
    ) acquires AllowedValidators, PontCoinCapabilities, OwnerCapability, StakePool, ValidatorConfig, ValidatorPerformance, ValidatorSet {
    ...
    #[test(pont_framework = @0x1, validator_1 = @0x123, validator_2 = @0x234)]
    #[expected_failure(abort_code = 0x10007)]
    public entry fun test_pending_active_validator_cannot_add_stake_if_exceeding_max_allowed(
        ...
        initialize_for_test_custom(pont_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 100000);
    ...
    #[test(pont_framework = @pont_framework, validator_1 = @0x123, validator_2 = @0x234)]
    #[expected_failure(abort_code = 0x10007)]
    ...
    public entry fun test_end_to_end(
        pont_framework: &signer,
        validator: &signer,
    ) acquires AllowedValidators, OwnerCapability, StakePool, PontCoinCapabilities, ValidatorConfig, ValidatorPerformance, ValidatorSet {
        initialize_for_test(pont_framework);
        initialize_test_validator(validator, 100, true, true);
        ...
        assert!(coin::balance<PontCoin>(validator_address) == 800, 2);
        ...
        assert!(coin::balance<PontCoin>(validator_address) == 850, 6);
        assert_validator_state(validator_address, 102, 51, 0, 0, 0);
        withdraw(validator, 51);
        assert!(coin::balance<PontCoin>(validator_address) == 901, 7);
    ...
    public entry fun test_cannot_reduce_lockup(
        ...
        staking_config::update_recurring_lockup_duration_secs(pont_framework, 1);
    ...
    public entry fun test_inactive_validator_cannot_join_if_exceed_increase_limit(
        ...
        initialize_for_test_custom(pont_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 50);
    ...
    public entry fun test_pending_active_validator_can_add_more_stake(
        ...
        initialize_for_test_custom(pont_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 10000);
    ...
    public entry fun test_pending_active_validator_cannot_add_more_stake_than_limit(
        ...
        initialize_for_test_custom(pont_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 100);
        // Need 1 validator to be in the active validator set so joining limit works.
        initialize_test_validator(validator_1, 100, true, true);
    ...
    public entry fun test_pending_active_validator_leaves_validator_set(
        ...
        assert!(borrow_global<ValidatorSet>(@pont_framework).total_joining_power == 100, 0);
        ...
        assert!(borrow_global<ValidatorSet>(@pont_framework).total_joining_power == 0, 1);
    }
    ...
    public entry fun test_active_validator_withdraw_should_cap_by_inactive_stake(
        ...
        assert!(coin::balance<PontCoin>(validator_address) == 1001, 2);
    ...
    #[test(pont_framework = @pont_framework, validator = @0x123, validator_2 = @0x234)]
    ...
    #[test(pont_framework = @0x1, validator_1 = @0x123, validator_2 = @0x234, validator_3 = @0x345)]
    public entry fun test_multiple_validators_join_and_leave(
        ...
        initialize_for_test_custom(pont_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 100);
        initialize_test_validator(validator_1, 100, false, false);
        initialize_test_validator(validator_2, 100, false, false);
        ...
        assert!(vector::borrow(&borrow_global<ValidatorSet>(@pont_framework).pending_inactive, 0).addr == validator_2_address, 0);
        ...
        assert!(vector::borrow(&borrow_global<ValidatorSet>(@pont_framework).pending_active, 0).addr == validator_3_address, 0);
        assert!(vector::borrow(&borrow_global<ValidatorSet>(@pont_framework).active_validators, 0).config.consensus_pubkey == CONSENSUS_KEY_1, 0);
        ...
        assert!(vector::borrow(&borrow_global<ValidatorSet>(@pont_framework).active_validators, 0).config.consensus_pubkey == CONSENSUS_KEY_2, 0);
    ...
    public entry fun test_delegated_staking_with_owner_cap(
        ...
        // Cleanups.
        coin::register<PontCoin>(validator);
    ...
    public entry fun test_validator_cannot_join_post_genesis(
        ...
        initialize_for_test_custom(pont_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, false, 1, 100, 100);
    ...
    #[test(
        pont_framework = @pont_framework,
        validator_1 = @pont_framework,
        validator_2 = @0x2,
        validator_3 = @0x3,
        validator_4 = @0x4,
        validator_5 = @0x5
    ...
    public entry fun test_invalid_config(
        pont_framework: &signer,
        validator: &signer,
    ) acquires AllowedValidators, PontCoinCapabilities, OwnerCapability, StakePool, ValidatorConfig, ValidatorSet {
        initialize_for_test_custom(pont_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 100);
    ...
    public entry fun test_removing_validator_from_active_set(
        ...
        initialize_test_validator(validator_1, 100, true, false);
        initialize_test_validator(validator_2, 100, true, true);
        assert!(vector::length(&borrow_global<ValidatorSet>(@pont_framework).active_validators) == 2, 0);
        ...
        remove_validators(pont_framework, &vector[validator_to_remove]);
        assert!(vector::length(&borrow_global<ValidatorSet>(@pont_framework).active_validators) == 1, 0);
        assert!(get_validator_state(validator_to_remove) == VALIDATOR_STATUS_PENDING_INACTIVE, 1);
    }

    #[test_only]
    public fun end_epoch() acquires StakePool, PontCoinCapabilities, ValidatorConfig, ValidatorPerformance, ValidatorSet {
    ...
    #[test(pont_framework = @0x1, validator = @0x123)]
    public entry fun test_allowed_validators(
        pont_framework: &signer,
        validator: &signer,
    ) acquires AllowedValidators, OwnerCapability, StakePool, ValidatorSet {
        let addr = signer::address_of(validator);
        configure_allowed_validators(pont_framework, vector[addr]);
    ...
    public entry fun test_not_allowed_validators(
        ...
        configure_allowed_validators(pont_framework, vector[]);
