...
module pont_framework::vesting {
    ...
    use pont_framework::account::{Self, SignerCapability, new_event_handle};
    use pont_framework::pont_account::assert_account_is_registered_for_apt;
    use pont_framework::pont_coin::PontCoin;
    use pont_framework::coin::{Self, Coin};
    use pont_framework::event::{EventHandle, emit_event};
    ...
    const VESTING_POOL_SALT: vector<u8> = b"pont_framework::vesting";
    ...
    public fun create_vesting_contract(
        ...
        buy_ins: SimpleMap<address, Coin<PontCoin>>,
        vesting_schedule: VestingSchedule,
        withdrawal_address: address,
        operator: address,
        voter: address,
    ...
    fun create_vesting_contract_account(
        ...
        coin::register<PontCoin>(&account_signer);
    ...
    fun withdraw_stake(vesting_contract: &VestingContract, contract_address: address): Coin<PontCoin> {
        ...
        let withdrawn_coins = coin::balance<PontCoin>(contract_address);
        let contract_signer = &get_vesting_account_signer_internal(vesting_contract);
        coin::withdraw<PontCoin>(contract_signer, withdrawn_coins)
    }
    ...
    #[test_only]
    use pont_framework::stake::with_rewards;

    #[test_only]
    use pont_framework::account::create_account_for_test;
    ...
    #[test_only]
    public entry fun setup(pont_framework: &signer, accounts: &vector<address>) {
        use pont_framework::pont_account::create_account;

        stake::initialize_for_test_custom(pont_framework, MIN_STAKE, GRANT_AMOUNT * 10, 3600, true, 10, 10000, 1000000);
    ...
    public fun setup_vesting_contract_with_schedule(
        ...
        let admin_address = signer::address_of(admin);
        let buy_ins = simple_map::create<address, Coin<PontCoin>>();
        let i = 0;
    ...
    #[test(pont_framework = @0x1, admin = @0x123, shareholder_1 = @0x234, shareholder_2 = @0x345, withdrawal = @111)]
    public entry fun test_end_to_end(
        ...
        setup(
            pont_framework, &vector[admin_address, withdrawal_address, shareholder_1_address, shareholder_2_address]);
        ...
        distribute(contract_address);
        let shareholder_1_bal = coin::balance<PontCoin>(shareholder_1_address);
        let shareholder_2_bal = coin::balance<PontCoin>(shareholder_2_address);
        ...
        assert!(coin::balance<PontCoin>(shareholder_1_address) == shareholder_1_bal + pending_distribution / 4, 0);
        assert!(coin::balance<PontCoin>(shareholder_2_address) == shareholder_2_bal + pending_distribution * 3 / 4, 1);
        // Withdrawal address receives the left-over dust of 1 coin due to rounding error.
        assert!(coin::balance<PontCoin>(withdrawal_address) == 1, 0);
        ...
        let previous_bal = coin::balance<PontCoin>(withdrawal_address);
        admin_withdraw(admin, contract_address);
        assert!(coin::balance<PontCoin>(withdrawal_address) == previous_bal + withdrawn_amount, 0);
    }

    #[test(pont_framework = @0x1, admin = @0x123)]
    ...
    public entry fun test_create_vesting_contract_with_zero_grant_should_fail(
        ...
        let admin_address = signer::address_of(admin);
        setup(pont_framework, &vector[admin_address]);
    ...
    public entry fun test_create_empty_vesting_schedule_should_fail(pont_framework: &signer) {
        setup(pont_framework, &vector[]);
        create_vesting_schedule(vector[], 1, 1);
    }

    #[test(pont_framework = @0x1)]
    #[expected_failure(abort_code = 0x10003)]
    public entry fun test_create_vesting_schedule_with_zero_period_duration_should_fail(pont_framework: &signer) {
        setup(pont_framework, &vector[]);
    ...
    public entry fun test_create_vesting_schedule_with_invalid_vesting_start_should_fail(pont_framework: &signer) {
        setup(pont_framework, &vector[]);
    ...
    #[test(pont_framework = @0x1, admin = @0x123, shareholder = @0x234)]
    public entry fun test_vest_twice_should_not_double_count(
        ...
        setup(pont_framework, &vector[admin_address, shareholder_address]);
    ...
    #[test(pont_framework = @0x1, admin = @0x123, shareholder = @0x234, operator = @0x345)]
    public entry fun test_unlock_rewards_should_pay_commission_first(
        ...
        let shareholder_address = signer::address_of(shareholder);
        setup(pont_framework, &vector[admin_address, shareholder_address, operator_address]);
        let contract_address = setup_vesting_contract(
            admin, &vector[shareholder_address], &vector[GRANT_AMOUNT], admin_address, 0);
        ...
        assert!(coin::balance<PontCoin>(shareholder_address) == staker_rewards + 1, 0);
        assert!(coin::balance<PontCoin>(operator_address) == commission - 1, 1);
    }

    #[test(pont_framework = @0x1, admin = @0x123, operator = @0x345)]
    public entry fun test_update_operator_with_same_commission(
        ...
        let admin_address = signer::address_of(admin);
        let operator_address = signer::address_of(operator);
        setup(pont_framework, &vector[admin_address, @11, operator_address]);
        let contract_address = setup_vesting_contract(
    ...
    public entry fun test_set_beneficiary_should_send_distribution(
        ...
        let admin_address = signer::address_of(admin);
        setup(pont_framework, &vector[admin_address, @11]);
        let contract_address = setup_vesting_contract(
        ...
        let vested_amount = fraction(GRANT_AMOUNT, 3, 48);
        distribute(contract_address);
        let balance = coin::balance<PontCoin>(@11);
    ...
    public entry fun test_reset_beneficiary(
        ...
        let admin_address = signer::address_of(admin);
        setup(pont_framework, &vector[admin_address, @11, @12]);
        let contract_address = setup_vesting_contract(
            admin, &vector[@11], &vector[GRANT_AMOUNT], admin_address, 0);
        ...
        let vested_amount = fraction(GRANT_AMOUNT, 3, 48);
        distribute(contract_address);
        assert!(coin::balance<PontCoin>(@11) == vested_amount, 0);
        assert!(coin::balance<PontCoin>(@12) == 0, 1);
    }

    #[test(pont_framework = @0x1, admin = @0x123, resetter = @0x234)]
    ...
    #[test(pont_framework = @0x1, admin = @0x123, resetter = @0x234, random = @0x345)]
