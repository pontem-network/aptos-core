...
module pont_framework::staking_contract {
    use std::bcs;
    use std::error;
    use std::signer;
    use std::vector;
    ...
    public entry fun create_staking_contract(
        ...
        let staked_coins = coin::withdraw<PontCoin>(staker, amount);
    ...
    #[test_only]
    public fun setup(pont_framework: &signer, staker: &signer, operator: &signer, initial_balance: u64) {
        // Reward rate of 0.1% per epoch.
        stake::initialize_for_test_custom(pont_framework, INITIAL_BALANCE, MAXIMUM_STAKE, 3600, true, 10, 10000, 1000000);
    ...
    public fun setup_staking_contract(
        ...
        setup(pont_framework, staker, operator, amount);
        let operator_address = signer::address_of(operator);
    ...
    #[test(pont_framework = @0x1, staker = @0x123, operator = @0x234)]
    public entry fun test_end_to_end(pont_framework: &signer, staker: &signer, operator: &signer) acquires Store {
        setup_staking_contract(pont_framework, staker, operator, INITIAL_BALANCE, 10);
        let staker_address = signer::address_of(staker);
        let operator_address = signer::address_of(operator);
        ...
        let operator_balance = coin::balance<PontCoin>(operator_address);
        ...
        distribute(staker_address, operator_address);
        operator_balance = coin::balance<PontCoin>(operator_address);
        ...
        let staker_balance = coin::balance<PontCoin>(staker_address);
    ...
    public entry fun test_operator_cannot_request_same_commission_multiple_times(
        pont_framework: &signer, staker: &signer, operator: &signer) acquires Store {
        setup_staking_contract(pont_framework, staker, operator, INITIAL_BALANCE, 10);
        let staker_address = signer::address_of(staker);
        let operator_address = signer::address_of(operator);
        let pool_address = stake_pool_address(staker_address, operator_address);
    ...
    public entry fun test_staker_cannot_create_staking_contract_with_invalid_commission(
        ...
        setup_staking_contract(pont_framework, staker, operator, INITIAL_BALANCE, 101);
    }
    ...
    public entry fun test_staker_cannot_create_staking_contract_with_less_than_min_stake_required(
        ...
        setup_staking_contract(pont_framework, staker, operator, 50, 100);
    }
    ...
    #[test(pont_framework = @0x1, staker = @0x123, operator_1 = @0x234, operator_2 = @0x345)]
    public entry fun test_staker_can_switch_operator(
        ...
        setup_staking_contract(pont_framework, staker, operator_1, INITIAL_BALANCE, 10);
        ...
        let operator_1_balance = coin::balance<PontCoin>(operator_1_address);
        ...
        let operator_2_balance = coin::balance<PontCoin>(operator_2_address);
        new_balance = with_rewards(new_balance);
