spec pont_framework::block {
    spec module {
        use pont_std::chain_status;
        // After genesis, `BlockResource` exist.
        invariant [suspendable] chain_status::is_operating() ==> exists<BlockResource>(@pont_framework);
    }
    ...
    spec emit_genesis_block_event {
        ...
        requires chain_status::is_operating();
        requires system_addresses::is_vm(vm);
        requires event::counter(global<BlockResource>(@pont_framework).new_block_events) == 0;
