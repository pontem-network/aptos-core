...
module pont_framework::aggregator_factory {
    use std::error;

    use pont_framework::system_addresses;
    use pont_std::aggregator::Aggregator;
    use pont_std::table::{Self, Table};

    friend pont_framework::genesis;
    friend pont_framework::optional_aggregator;
    ...
    public(friend) fun create_aggregator_internal(limit: u128): Aggregator acquires AggregatorFactory {
        assert!(
            exists<AggregatorFactory>(@pont_framework),
            error::not_found(EAGGREGATOR_FACTORY_NOT_FOUND)
        );

        let aggregator_factory = borrow_global_mut<AggregatorFactory>(@pont_framework);
    ...
    public fun create_aggregator(account: &signer, limit: u128): Aggregator acquires AggregatorFactory {
        // Only Pont Framework (0x1) account can call this for now.
    ...
    #[test_only]
    public fun initialize_aggregator_factory_for_test(pont_framework: &signer) {
        initialize_aggregator_factory(pont_framework);
    }
}
