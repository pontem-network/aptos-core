...
/// }
/// ```
module pont_framework::resource_account {
    use std::error;
    use std::signer;
    use std::vector;
    ...
    public entry fun create_resource_account_and_fund(
        ...
        let (resource, resource_signer_cap) = account::create_resource_account(origin, seed);
        coin::register<PontCoin>(&resource);
        coin::transfer<PontCoin>(origin, signer::address_of(&resource), fund_amount);
        rotate_account_authentication_key_and_store_capability(
            origin,
            resource,
            resource_signer_cap,
    ...
    public entry fun create_resource_account_and_publish_package(
        ...
        let (resource, resource_signer_cap) = account::create_resource_account(origin, seed);
        pont_framework::code::publish_package_txn(&resource, metadata_serialized, code);
        rotate_account_authentication_key_and_store_capability(
            origin,
            resource,
            resource_signer_cap,
    ...
    public entry fun test_create_account_and_retrieve_cap(user: signer) acquires Container {
        ...
        let resource_addr = pont_framework::account::create_resource_address(&user_addr, seed);
    ...
    public entry fun with_coin(framework: signer, user: signer) acquires Container {
        let user_addr = signer::address_of(&user);
        pont_framework::pont_account::create_account(copy user_addr);

        let (burn, mint) = pont_framework::pont_coin::initialize_for_test(&framework);
        let coin = coin::mint<PontCoin>(100, &mint);
        ...
        let resource_addr = pont_framework::account::create_resource_address(&user_addr, seed);
        coin::transfer<PontCoin>(&user, resource_addr, 10);
    ...
    public entry fun without_coin(framework: signer, user: signer) acquires Container {
        let user_addr = signer::address_of(&user);
        pont_framework::pont_account::create_account(user_addr);
