...
module pont_framework::consensus_config {
    use std::error;
    use std::vector;
    ...
    public(friend) fun initialize(pont_framework: &signer, config: vector<u8>) {
        system_addresses::assert_pont_framework(pont_framework);
        assert!(vector::length(&config) > 0, error::invalid_argument(EINVALID_CONFIG));
        move_to(pont_framework, ConsensusConfig { config });
    }
    ...
    public fun set(account: &signer, config: vector<u8>) acquires ConsensusConfig {
        ...
        let config_ref = &mut borrow_global_mut<ConsensusConfig>(@pont_framework).config;
        *config_ref = config;
