...
module pont_framework::reconfiguration {
    use std::error;
    use std::signer;

    use pont_framework::account;
    use pont_framework::event;
    ...
    friend pont_framework::consensus_config;
    friend pont_framework::features;
    friend pont_framework::gas_schedule;
    friend pont_framework::genesis;
    friend pont_framework::version;
    ...
    /// Publishes `Configuration` resource. Can only be invoked by pont framework account, and only a single time in Genesis.
    public(friend) fun initialize(pont_framework: &signer) {
        ...
        assert!(account::get_guid_next_creation_num(signer::address_of(pont_framework)) == 2, error::invalid_state(EINVALID_GUID_FOR_EVENT));
        move_to<Configuration>(
            pont_framework,
            Configuration {
                epoch: 0,
                last_reconfiguration_time: 0,
                events: account::new_event_handle<NewEpochEvent>(pont_framework),
            }
        );
    }
    ...
    fun disable_reconfiguration(pont_framework: &signer) {
        system_addresses::assert_pont_framework(pont_framework);
        assert!(reconfiguration_enabled(), error::invalid_state(ECONFIGURATION));
        move_to(pont_framework, DisableReconfiguration {})
    }

    /// Private function to resume reconfiguration.
    /// This function should only be used for offline WriteSet generation purpose and should never be invoked on chain.
    fun enable_reconfiguration(pont_framework: &signer) acquires DisableReconfiguration {
        system_addresses::assert_pont_framework(pont_framework);

        assert!(!reconfiguration_enabled(), error::invalid_state(ECONFIGURATION));
        DisableReconfiguration {} = move_from<DisableReconfiguration>(signer::address_of(pont_framework));
    }

    fun reconfiguration_enabled(): bool {
        !exists<DisableReconfiguration>(@pont_framework)
    }
    ...
    public(friend) fun reconfigure() acquires Configuration {
        ...
        let config_ref = borrow_global_mut<Configuration>(@pont_framework);
    ...
    public fun last_reconfiguration_time(): u64 acquires Configuration {
        borrow_global<Configuration>(@pont_framework).last_reconfiguration_time
    }

    public fun current_epoch(): u64 acquires Configuration {
        borrow_global<Configuration>(@pont_framework).epoch
    }
    ...
    public fun initialize_for_test(account: &signer) {
        system_addresses::assert_pont_framework(account);
        move_to<Configuration>(
            account,
            Configuration {
                epoch: 0,
