spec pont_framework::system_addresses {
    ...
    spec assert_pont_framework {
        pragma opaque;
        include AbortsIfNotPontFramework;
    }

    /// Specifies that a function aborts if the account does not have the pont framework address.
    spec schema AbortsIfNotPontFramework {
        account: signer;
        aborts_if signer::address_of(account) != @pont_framework with error::PERMISSION_DENIED;
    }
