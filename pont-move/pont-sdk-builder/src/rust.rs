...
/// If `local_types` is true, we generate a file suitable for the Pont codebase itself
...
struct RustEmitter<T> {
    ...
    /// Whether we are targetting the Pont repository itself (as opposed to generated Pont types).
    local_types: bool,
}
    ...
    fn get_external_definitions(local_types: bool) -> serde_generate::ExternalDefinitions {
        let definitions = if local_types {
            vec![
                ...
                (
                    "pont_types::transaction",
                    vec!["TransactionPayload", "EntryFunction"],
                ),
                ("pont_types::account_address", vec!["AccountAddress"]),
            ]
        } else {
            vec![(
                "pont_types",
...
pub fn decode(payload: &TransactionPayload) -> Option<EntryFunctionCall> {{
    ...
    fn output_transaction_script_name_method(
        ...
        writeln!(
            self.out,
            r#"
/// Return the name of an Pont `Script` from a structured object `ScriptCall`.
...
pub struct Installer {
    install_dir: PathBuf,
    pont_types_version: String,
}

impl Installer {
    pub fn new(install_dir: PathBuf, pont_types_version: String) -> Self {
        Installer {
            install_dir,
            pont_types_version,
        }
...
serde_bytes = "0.11.6"
pont-types = {{ path = "../pont-types", version = "{}" }}
"#,
            name, version, self.pont_types_version,
        )?;
