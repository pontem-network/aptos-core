...
pub fn output(
    ...
    pont_module_path: Option<String>,
    package_name: String,
    abis: &[EntryABI],
) -> Result<()> {
    ...
    T: Write,
{
    fn output_script_call_enum_with_imports(&mut self, abis: &[EntryABI]) -> Result<()> {
        let pont_types_package = match &self.pont_module_path {
            Some(path) => format!("{}/ponttypes", path),
            None => "ponttypes".into(),
        };
        let mut external_definitions = crate::common::get_external_definitions(&pont_types_package);
    ...
    fn output_encode_method(&mut self, abis: &[EntryABI]) -> Result<()> {
        ...
        if !transaction_script_abis.is_empty() {
            writeln!(
                self.out,
                r#"
// Build an Pont `Script` from a structured object `ScriptCall`.
func EncodeScript(call ScriptCall) ponttypes.Script {{"#
            )?;
            self.out.indent();
        ...
        if !entry_fun_abis.is_empty() {
            writeln!(
                self.out,
                r#"
// Build an Pont `TransactionPayload` from a structured object `EntryFunctionCall`.
func EncodeEntryFunction(call EntryFunctionCall) ponttypes.TransactionPayload {{"#
            )?;
            self.out.indent();
            writeln!(self.out, "switch call := call.(type) {{")?;
    ...
    fn output_transaction_script_decode_method(&mut self) -> Result<()> {
        writeln!(
            self.out,
            r#"
// Try to recognize an Pont `Script` and convert it into a structured object `ScriptCall`.
func DecodeScript(script *ponttypes.Script) (ScriptCall, error) {{
    ...
    fn output_entry_function_decode_method(&mut self) -> Result<()> {
        writeln!(
            self.out,
            r#"
// Try to recognize an Pont `TransactionPayload` and convert it into a structured object `EntryFunctionCall`.
func DecodeEntryFunctionPayload(script ponttypes.TransactionPayload) (EntryFunctionCall, error) {{
    switch script := script.(type) {{
        case *ponttypes.TransactionPayload__EntryFunction:
    ...
    fn output_transaction_script_encoder_function(
        ...
        writeln!(
            self.out,
            r#"return ponttypes.Script {{
	Code: append([]byte(nil), {}_code...),
	TyArgs: []ponttypes.TypeTag{{{}}},
	Args: []ponttypes.TransactionArgument{{{}}},
}}"#,
            abi.name(),
    ...
    fn output_entry_function_encoder_function(&mut self, abi: &EntryFunctionABI) -> Result<()> {
        writeln!(
            self.out,
            "\n{}\nfunc Encode{}{}({}) ponttypes.TransactionPayload {{",
            Self::quote_doc(abi.doc()),
            abi.module_name().name().to_string().to_camel_case(),
            abi.name().to_camel_case(),
            [
        ...
        writeln!(
            self.out,
            r#"return &ponttypes.TransactionPayload__EntryFunction {{
            ponttypes.EntryFunction {{
                Module: {},
                Function: {},
                TyArgs: []ponttypes.TypeTag{{{}}},
                Args: [][]byte{{{}}},
    ...
    fn output_transaction_script_decoder_function(
        ...
        writeln!(
            self.out,
            "\nfunc decode_{}(script *ponttypes.Script) (ScriptCall, error) {{",
            abi.name(),
        )?;
        self.out.indent();
        writeln!(
...
"#,
    ...
    fn output_entry_function_decoder_function(&mut self, abi: &EntryFunctionABI) -> Result<()> {
        writeln!(
            self.out,
            "\nfunc decode_{}_{}(script ponttypes.TransactionPayload) (EntryFunctionCall, error) {{",
...
"#,
    ...
    fn output_transaction_script_decoder_map(
        ...
        writeln!(
            self.out,
            r#"
var script_decoder_map = map[string]func(*ponttypes.Script) (ScriptCall, error) {{"#
        )?;
        self.out.indent();
        for abi in abis {
    ...
    fn output_entry_function_decoder_map(&mut self, abis: &[EntryFunctionABI]) -> Result<()> {
        writeln!(
            self.out,
            r#"
var entry_function_decoder_map = map[string]func(ponttypes.TransactionPayload) (EntryFunctionCall, error) {{"#
        )?;
        self.out.indent();
        for abi in abis {
            writeln!(
...
"#,
    ...
    fn output_decoding_helper(&mut self, type_tag: &TypeTag) -> Result<()> {
        ...
        writeln!(
            self.out,
            r#"
func decode_{0}_argument(arg ponttypes.TransactionArgument) (value {1}, err error) {{
	if arg, ok := arg.(*ponttypes.TransactionArgument__{2}); ok {{
		{3}
	}} else {{
...
"#,
    ...
    fn quote_module_id(module_id: &ModuleId) -> String {
        format!(
            "ponttypes.ModuleId {{ Address: {}, Name: {} }}",
    ...
    fn quote_type_parameters(ty_args: &[TypeArgumentABI]) -> Vec<String> {
        ty_args
            .iter()
            .map(|ty_arg| format!("{} ponttypes.TypeTag", ty_arg.name()))
            .collect()
    }
    ...
    fn quote_type(type_tag: &TypeTag) -> String {
        ...
        match type_tag {
            ...
            Address => "ponttypes.AccountAddress".into(),
            Vector(type_tag) => {
    ...
    fn quote_transaction_argument_for_script(type_tag: &TypeTag, name: &str) -> String {
        use TypeTag::*;
        match type_tag {
            Bool => format!("(*ponttypes.TransactionArgument__Bool)(&{})", name),
            U8 => format!("(*ponttypes.TransactionArgument__U8)(&{})", name),
            U64 => format!("(*ponttypes.TransactionArgument__U64)(&{})", name),
            U128 => format!("(*ponttypes.TransactionArgument__U128)(&{})", name),
            Address => format!("&ponttypes.TransactionArgument__Address{{{}}}", name),
            Vector(type_tag) => match type_tag.as_ref() {
                U8 => format!("(*ponttypes.TransactionArgument__U8Vector)(&{})", name),
                _ => common::type_not_allowed(type_tag),
            },
            Struct(_) | Signer => common::type_not_allowed(type_tag),
        }
