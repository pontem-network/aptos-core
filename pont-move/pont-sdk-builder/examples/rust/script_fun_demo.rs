...
use framework::{pont_coin_transfer, EntryFunctionCall};
use pont_types::AccountAddress;
...
fn demo_p2p_entry_function() {
    ...
    let payload = pont_coin_transfer(payee.clone(), amount);
    let function_call = EntryFunctionCall::decode(&payload);
    match function_call {
        Some(EntryFunctionCall::PontCoinTransfer { amount: a, to: p }) => {
            assert_eq!(a, amount);
