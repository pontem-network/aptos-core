...
use pont_types::account_config::TransactionValidation;
...
pub fn convert_prologue_error(
    ...
    Err(match status {
            ...
            if !transaction_validation.is_account_module_abort(&location) =>
        {
            ...
            error!(
                *log_context,
                "[pont_vm] Unexpected prologue Move abort: {:?}::{:?} (Category: {:?} Reason: {:?})",
        ...
        status @ VMStatus::ExecutionFailure { .. } | status @ VMStatus::Error(_) => {
            ...
            error!(
                *log_context,
                "[pont_vm] Unexpected prologue error: {:?}", status
            );
            VMStatus::Error(StatusCode::UNEXPECTED_ERROR_FROM_KNOWN_MOVE_FUNCTION)
        }
    })
...
pub fn convert_epilogue_error(
    ...
    Err(match status {
            ...
            if !transaction_validation.is_account_module_abort(&location) =>
        {
            ...
            error!(
                *log_context,
                "[pont_vm] Unexpected success epilogue Move abort: {:?}::{:?} (Category: {:?} Reason: {:?})",
                location, code, category, reason,
            );
            VMStatus::Error(StatusCode::UNEXPECTED_ERROR_FROM_KNOWN_MOVE_FUNCTION)
        }
        ...
        status => {
            ...
            error!(
                *log_context,
                "[pont_vm] Unexpected success epilogue error: {:?}", status,
            );
            VMStatus::Error(StatusCode::UNEXPECTED_ERROR_FROM_KNOWN_MOVE_FUNCTION)
        }
    })
...
pub fn expect_only_successful_execution(
    ...
    Err(match status {
        ...
        status => {
            ...
            error!(
                *log_context,
                "[pont_vm] Unexpected error from known Move function, '{}'. Error: {:?}",
                function_name,
                status,
            );
            VMStatus::Error(StatusCode::UNEXPECTED_ERROR_FROM_KNOWN_MOVE_FUNCTION)
