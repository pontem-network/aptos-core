...
use crate::{
    ...
    parallel_executor::vm_wrapper::PontVMWrapper,
    pont_vm::PontVM,
};
use move_core_types::vm_status::{StatusCode, VMStatus};
use pont_aggregator::{delta_change_set::DeltaOp, transaction::TransactionOutputExt};
...
pub(crate) struct PontTransactionOutput(TransactionOutputExt);

impl PontTransactionOutput {
...
impl PTransactionOutput for PontTransactionOutput {
...
impl ParallelPontVM {
    pub fn execute_block<S: StateView>(
        ...
        let signature_verified_block: Vec<PreprocessedTransaction> = transactions
            .par_iter()
            .map(|txn| preprocess_transaction::<PontVM>(txn.clone()))
            .collect();

        match ParallelTransactionExecutor::<PreprocessedTransaction, PontVMWrapper<S>>::new(
            concurrency_level,
        )
        .execute_transactions_parallel(state_view, signature_verified_block)
        {
            Ok((results, delta_resolver)) => {
                ...
                for res in results.iter() {
                    let output_ext = PontTransactionOutput::as_ref(res);
                ...
                Ok((
                    results
                        ...
                        .map(|(res, delta_writes)| {
                            let output_ext = PontTransactionOutput::into(res);
            ...
            Err(err @ Error::ModulePathReadWrite) => {
                let output = PontVM::execute_block_and_keep_vm_status(transactions, state_view)?;
                Ok((
                    output
                        .into_iter()
