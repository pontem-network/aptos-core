...
use pont_aggregator::transaction::TransactionOutputExt;
use pont_gas::{
    AbstractValueSizeGasParameters, FromOnChainGasSchedule, Gas, NativeGasParameters,
    PontGasParameters, StorageGasParameters,
};
use pont_logger::prelude::*;
use pont_state_view::StateView;
use pont_types::on_chain_config::{FeatureFlag, Features};
use pont_types::transaction::AbortInfo;
use pont_types::{
...
pub struct PontVMImpl {
    move_vm: Arc<MoveVmExt>,
    gas_feature_version: u64,
    gas_params: Option<PontGasParameters>,
...
impl PontVMImpl {
    ...
    pub fn new<S: StateView>(state: &S) -> Self {
        ...
        let (mut gas_params, gas_feature_version): (Option<PontGasParameters>, u64) =
            match GasScheduleV2::fetch_config(&storage) {
                Some(gas_schedule) => {
                    let feature_version = gas_schedule.feature_version;
                    let map = gas_schedule.to_btree_map();
                    (
                        PontGasParameters::from_on_chain_gas_schedule(&map),
                        feature_version,
                    )
                }
                None => match GasSchedule::fetch_config(&storage) {
                    Some(gas_schedule) => {
                        let map = gas_schedule.to_btree_map();
                        (PontGasParameters::from_on_chain_gas_schedule(&map), 0)
                    }
                    None => (None, 0),
                },
            };
    ...
    ) -> Result<&PontGasParameters, VMStatus> {
...
#[derive(Clone, Copy)]
pub struct PontVMInternals<'a>(&'a PontVMImpl);

impl<'a> PontVMInternals<'a> {
    pub fn new(internal: &'a PontVMImpl) -> Self {
        Self(internal)
    }
    ...
    pub fn gas_params(
        self,
        log_context: &AdapterLogSchema,
    ) -> Result<&'a PontGasParameters, VMStatus> {
