...
use crate::{
    ...
    pont_vm_impl::{get_transaction_output, PontVMImpl, PontVMInternals},
    system_module_names::*,
...
use pont_gas::PontGasMeter;
use pont_logger::prelude::*;
use pont_module_verifier::module_init::verify_module_init_function;
...
#[derive(Clone)]
pub struct PontVM(pub(crate) PontVMImpl);

struct PontSimulationVM(PontVM);

impl PontVM {
    pub fn new<S: StateView>(state: &S) -> Self {
        Self(PontVMImpl::new(state))
    }
    ...
    pub fn internals(&self) -> PontVMInternals {
        PontVMInternals::new(&self.0)
    }
    ...
    pub(crate) fn execute_user_transaction<S: MoveResolverExt + StateView>(
        ...
        let mut gas_meter = PontGasMeter::new(
    ...
    fn validate_waypoint_change_set(
        ...
        if has_new_block_event && has_new_epoch_event {
            ...
            error!(
                *log_context,
                "[pont_vm] waypoint txn needs to emit new epoch and block"
            );
    ...
    pub(crate) fn process_waypoint_change_set<S: MoveResolverExt + StateView>(
        ...
        let change_set_ext = match self.execute_writeset(
            storage,
            &writeset_payload,
            Some(pont_types::account_config::reserved_vm_address()),
    ...
    pub fn execute_block_and_keep_vm_status(
        ...
        let vm = PontVM::new(&state_view_cache);
    ...
    pub fn simulate_signed_transaction(
        ...
        let vm = PontVM::new(state_view);
        let simulation_vm = PontSimulationVM(vm);
...
impl VMExecutor for PontVM {
    ...
    fn execute_block(
        ...
        if concurrency_level > 1 {
            let (result, err) = crate::parallel_executor::ParallelPontVM::execute_block(
                transactions,
                state_view,
                concurrency_level,
            )?;
...
impl VMAdapter for PontVM {
    ...
    fn should_restart_execution(vm_output: &TransactionOutput) -> bool {
        let new_epoch_event_key = pont_types::on_chain_config::new_epoch_event_key();
        vm_output
            .events()
            .iter()
    ...
    fn execute_single_transaction<S: MoveResolverExt + StateView>(
        ...
        Ok(match txn {
            PreprocessedTransaction::BlockMetadata(block_metadata) => {
                fail_point!("pont_vm::execution::block_metadata");
            ...
            PreprocessedTransaction::UserTransaction(txn) => {
                    ...
                    vm_status.clone().keep_or_discard()
                {
                    error!(
                        *log_context,
                        "[pont_vm] Transaction breaking invariant violation. txn: {:?}",
...
impl AsRef<PontVMImpl> for PontVM {
    fn as_ref(&self) -> &PontVMImpl {
        &self.0
    }
}

impl AsMut<PontVMImpl> for PontVM {
    fn as_mut(&mut self) -> &mut PontVMImpl {
        &mut self.0
    }
}

impl PontSimulationVM {
