...
use crate::{assert_success, PontPackageHooks};
...
use pont::move_tool::MemberId;
use pont_crypto::ed25519::Ed25519PrivateKey;
use pont_crypto::{PrivateKey, Uniform};
use pont_gas::{InitialGasSchedule, PontGasParameters, ToOnChainGasSchedule};
use pont_types::on_chain_config::{FeatureFlag, GasScheduleV2};
use pont_types::transaction::TransactionOutput;
use pont_types::{
...
impl MoveHarness {
    ...
    pub fn new() -> Self {
        register_package_hooks(Box::new(PontPackageHooks {}));
        Self {
    ...
    pub fn new_account_at(&mut self, addr: AccountAddress) -> Account {
        ...
        // Mint the account 10M Pont coins (with 8 decimals).
    ...
    /// Gets the account where the Pont framework is installed (0x1).
    pub fn pont_framework_account(&mut self) -> Account {
    ...
    pub fn enable_features(&mut self, enabled: Vec<FeatureFlag>, disabled: Vec<FeatureFlag>) {
        let acc = self.pont_framework_account();
    ...
    pub fn increase_transaction_size(&mut self) {
        // TODO: The PontGasParameters::zeros() schedule doesn't do what we want, so
        ...
        let entries = PontGasParameters::initial().to_on_chain_gas_schedule();
        let entries = entries
            .into_iter()
            .map(|(name, val)| {
...
macro_rules! assert_abort {
    ($s:expr, $c:pat) => {{
        assert!(matches!(
            $s,
            pont_types::transaction::TransactionStatus::Keep(
                pont_types::transaction::ExecutionStatus::MoveAbort { code: $c, .. }
            ),
        ));
    }};
}
