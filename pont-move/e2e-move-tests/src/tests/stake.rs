...
fn test_staking_mainnet() {
    ...
    // Verify that pont framework account cannot mint coins.
    let pont_framework_account = harness.new_account_at(CORE_CODE_ADDRESS);
    assert_abort!(
        harness.run_transaction_payload(
            &pont_framework_account,
            pont_stdlib::pont_coin_mint(CORE_CODE_ADDRESS, 1000),
        ),
        _
    );
