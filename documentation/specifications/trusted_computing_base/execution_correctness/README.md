# Pont Execution Correctness Specification

## Abstract

This specification outlines the design and implementation of Pont Execution Correctness (LEC): a secured service
dedicated for executing transactions correctly (with the MOVE VM) and used by Consensus in the Pont Blockchain.
...
a secure and audited execution service is necessary to guarantee this statement. In Pont Core, it is LEC that plays
...
* **commit**: Executor is the only module in Pont core that has write access to Storage. Once a new state has reached consensus across the quorum, it is Executor that commits the agreed state to Storage.

Let's review the system flow chart to understand how LEC works with other Pont core services. The diagram below arguments the architecture of a part of Pont core system that relevant to LEC with numbered steps showing the interations between LEC and the other components.
...
9. LEC sends the commit command to Storage with necessary data, persisting the change to the Pont blockchain.
...
Storage of Pont Core is implemented as `PontDB`, a rocksdb based storage system highly customized for
Pont. It stores all the required data consistutes the Pont blockchain. After executing a block, LEC will store the
...
/// expected of a Pont DB. This adds write APIs to DbReader.
