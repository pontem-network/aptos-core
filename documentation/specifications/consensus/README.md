...
The consensus specification describes the mechanism used by the Pont validators to agree on both ordering and transaction execution output under byzantine fault-tolerant (BFT) conditions - at most _f_ validators (where _f_ < (_all validators_)/3) are faulty or malicious. Currently, the consensus specification is an implementation of _PontBFT_, a BFT consensus protocol that ensures liveness and safety under partial synchrony. The [PontBFT whitepaper](https://pont.dev/docs/state-machine-replication-paper) describes a high level overview of the protocol, the liveness and safety proofs, and a rationale on why PontBFT was adopted for the LPN. This document specifies how to implement the PontBFT protocol in order to participate as a validating node in the LPN.
...
All network communication occurs over PontNet and any serialization, deserialization and hashing is determined by [BCS](https://docs.rs/bcs/).
...
* `safety_rules`: the voting and commit rule of PontBFT, specified in [safety rules](#safety-rules).
...
* the Pont network is bootstrapped with a genesis transaction (note the different from genesis block) and a ledger_info commits it with next_epoch_state set.
