...
hashing, signing and verifying throughout the Pont Blockchain, as well as
...
through [Pont Improvement Processes](https://github.com/aptos-labs/aip) (AIPs).

## Algorithms

### Signature and verification

Pont uses EdDSA with Curve25519 as its workhorse algorithm, both internally
...
Pont's implementation of EdDSA makes choices and slight variations from these
standards in ways that we make precise below:

* Pont uses the PureEdDSA variant of Ed25519 described in RFC 8032. Indeed,
...
Pont a uses cofactor-less verification equation. We note that cofactor-less
...
to comply with a Pont specification that would mandate cofactored verification
...
input and output are for consumption by Pont code and data structures.

Pont also uses other hashing functions when conforming to an external
...
exposed to the Pont developer.

## Domain separation

In Pont, we are signing large values (transactions, block proposals), which
...
produce the same hash. However, when we hash Pont values, we hash their
...
moment, suffice to say that for every Pont data structure, we also define
and specify a domain separator.

More specifically, the domain separator [starts with a
prefix](https://github.com/aptos-labs/pont-core/blob/ca14568db9df8c7008dfc6698392ef479e013cf6/crypto/crypto/src/hash.rs#L114-L117)
...
specification](https://github.com/aptos-labs/pont-core/tree/main/testsuite/generate-format/tests/staged),
including the trace name, from code, and developers are advised to look for
changes therein across Pont releases.
...
serialization library guarantees deterministic serialization for any Pont data
structure.
...
Signing a Pont data structure (including e.g. `RawTransaction` or
...
That is, if `v` is the Pont value we aim to sign, the bytes sent to the
...
In the Pont code base, any structure that implements the derivable
...
When defining a new data structure in the Pont code base, the developer should
...
Pont also supports a transaction signature format which implements
...
[(Authenticator)](https://github.com/aptos-labs/pont-core/blob/main/pont-move/framework/core/sources/Authenticator.move),
