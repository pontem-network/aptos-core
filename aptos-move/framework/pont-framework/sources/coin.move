...
module pont_framework::coin {
    ...
    use pont_framework::optional_aggregator::{Self, OptionalAggregator};
    use pont_framework::system_addresses;

    use pont_std::type_info;

    friend pont_framework::aptos_coin;
    ...
    public(friend) fun initialize_supply_config(pont_framework: &signer) {
        system_addresses::assert_pont_framework(pont_framework);
        move_to(pont_framework, SupplyConfig { allow_upgrades: false });
    }

    /// This should be called by on-chain governance to update the config and allow
    // or disallow upgradability of total supply.
    public fun allow_supply_upgrades(pont_framework: &signer, allowed: bool) acquires SupplyConfig {
        system_addresses::assert_pont_framework(pont_framework);
        let allow_upgrades = &mut borrow_global_mut<SupplyConfig>(@pont_framework).allow_upgrades;
    ...
    public entry fun upgrade_supply<CoinType>(account: &signer) acquires CoinInfo, SupplyConfig {
        ...
        assert!(
            borrow_global_mut<SupplyConfig>(@pont_framework).allow_upgrades,
    ...
    #[test(source = @0x2, framework = @pont_framework)]
    ...
    #[test(framework = @pont_framework, other = @0x123)]
