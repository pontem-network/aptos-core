...
module pont_framework::storage_gas {
    ...
    public(friend) fun set_config(pont_framework: &signer, config: StorageGasConfig) acquires StorageGasConfig {
        system_addresses::assert_pont_framework(pont_framework);
        *borrow_global_mut<StorageGasConfig>(@pont_framework) = config;
    }
    ...
    /// target utilization.
    public fun initialize(pont_framework: &signer) {
        system_addresses::assert_pont_framework(pont_framework);
        assert!(
            !exists<StorageGasConfig>(@pont_framework),
        ...
        move_to(pont_framework, StorageGasConfig {
            item_config,
            byte_config,
        });

        assert!(
            !exists<StorageGas>(@pont_framework),
            error::already_exists(ESTORAGE_GAS)
        );
        move_to(pont_framework, StorageGas {
            per_item_read: 300 * k,
            per_item_create: 5 * m,
    ...
    public(friend) fun on_reconfig() acquires StorageGas, StorageGasConfig {
        ...
        let gas_config = borrow_global<StorageGasConfig>(@pont_framework);
        let gas = borrow_global_mut<StorageGas>(@pont_framework);
    ...
    fun test_initialize_and_reconfig(framework: signer) acquires StorageGas, StorageGasConfig {
        ...
        on_reconfig();
        let gas_parameter = borrow_global<StorageGas>(@pont_framework);
