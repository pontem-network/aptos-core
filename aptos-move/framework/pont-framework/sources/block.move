...
module pont_framework::block {
    ...
    public(friend) fun initialize(pont_framework: &signer, epoch_interval_microsecs: u64) {
        ...
        move_to<BlockResource>(
            ...
            BlockResource {
                ...
                new_block_events: account::new_event_handle<NewBlockEvent>(pont_framework),
                update_epoch_interval_events: account::new_event_handle<UpdateEpochIntervalEvent>(pont_framework),
            }
        );
    }
    ...
    public fun update_epoch_interval_microsecs(
        ...
        let block_resource = borrow_global_mut<BlockResource>(@pont_framework);
    ...
    public fun get_epoch_interval_secs(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@pont_framework).epoch_interval / 1000000
    }
    ...
    fun block_prologue(
        ...
        let block_metadata_ref = borrow_global_mut<BlockResource>(@pont_framework);
    ...
    public fun get_current_block_height(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@pont_framework).height
    }
    ...
    public entry fun test_update_epoch_interval(pont_framework: signer) acquires BlockResource {
        account::create_account_for_test(@pont_framework);
        initialize(&pont_framework, 1);
        assert!(borrow_global<BlockResource>(@pont_framework).epoch_interval == 1, 0);
        update_epoch_interval_microsecs(&pont_framework, 2);
        assert!(borrow_global<BlockResource>(@pont_framework).epoch_interval == 2, 1);
    }

    #[test(pont_framework = @pont_framework, account = @0x123)]
