module pont_framework::system_addresses {
    use std::error;
    use std::signer;
    ...
    const ENOT_pont_framework_ADDRESS: u64 = 3;
    ...
    public fun assert_pont_framework(account: &signer) {
        assert!(
            is_pont_framework_address(signer::address_of(account)),
            error::permission_denied(ENOT_pont_framework_ADDRESS),
        )
    }
    ...
    public fun is_framework_reserved_address(addr: address): bool {
        is_pont_framework_address(addr) ||
            addr == @0x2 ||
            addr == @0x3 ||
            addr == @0x4 ||
            addr == @0x5 ||
    ...
    public fun is_pont_framework_address(addr: address): bool {
        addr == @pont_framework
    }
    ...
    public fun is_reserved_address(addr: address): bool {
        is_pont_framework_address(addr) || is_vm_address(addr)
    }
}
