...
module pont_framework::stake {
    ...
    public(friend) fun store_aptos_coin_mint_cap(pont_framework: &signer, mint_cap: MintCapability<AptosCoin>) {
        system_addresses::assert_pont_framework(pont_framework);
        move_to(pont_framework, AptosCoinCapabilities { mint_cap })
    }
    ...
    fun distribute_rewards(
        ...
        if (rewards_amount > 0) {
            let mint_cap = &borrow_global<AptosCoinCapabilities>(@pont_framework).mint_cap;
    ...
    public fun initialize_for_test_custom(
        ...
        if (!exists<AptosCoinCapabilities>(@pont_framework)) {
            let (burn_cap, mint_cap) = aptos_coin::initialize_for_test(pont_framework);
            store_aptos_coin_mint_cap(pont_framework, mint_cap);
