...
struct TestValidator {
    ...
    _db_path: pont_temppath::TempPath,
}

impl TestValidator {
    fn new() -> Self {
        let _db_path = pont_temppath::TempPath::new();
        _db_path.create_as_dir().unwrap();
        let (db, db_rw) = DbReaderWriter::wrap(PontDB::new_for_test(_db_path.path()));
        executor_test_helpers::bootstrap_genesis::<PontVM>(
            &db_rw,
...
//   language e2e tests in: pont-core/language/e2e-testsuite/src/tests/verify_txn.rs ->
...
// * SENDING_ACCOUNT_FROZEN: Tested in functional-tests/tests/pont_account/freezing.move.
...
fn test_validate_transaction() {
    ...
    let address = account_config::pont_test_root_address();
    let program = pont_stdlib::pont_coin_mint(address, 100);
...
fn test_validate_invalid_signature() {
    ...
    let address = account_config::pont_test_root_address();
    let program = pont_stdlib::pont_coin_transfer(address, 100);
...
fn test_get_account_sequence_number() {
    let vm_validator = TestValidator::new();
    let root_address = account_config::pont_test_root_address();
