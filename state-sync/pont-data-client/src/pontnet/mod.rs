...
use crate::{
    ...
    Error, GlobalDataSummary, PontDataClient, Response, ResponseCallback, ResponseContext,
...
use pont_config::{
    config::{BaseConfig, PontDataClientConfig, StorageServiceConfig},
...
// Useful constants for the Pont Data Client
...
/// An [`PontDataClient`] that fulfills requests from remote peers' Storage Service
/// over PontNet.
///
/// The `PontNetDataClient`:
///
/// 1. Sends requests to connected Pont peers.
...
/// and/or threads.
#[derive(Clone, Debug)]
pub struct PontNetDataClient {
    /// Config for PontNet data client.
    data_client_config: PontDataClientConfig,
    /// The underlying PontNet storage service client.
    ...
    fn get_all_connected_peers(&self) -> Result<Vec<PeerNetworkId>, Error> {
        ...
        if connected_peers.is_empty() {
            return Err(Error::DataIsUnavailable(
                "No connected PontNet peers!".to_owned(),
            ));
        }
        Ok(connected_peers)
    }
    ...
    async fn send_request_to_peer(
        ...
        match result {
            Ok(response) => {
                ...
                // with this RPC.
                let response_callback = PontNetResponseCallback {
...
#[async_trait]
impl PontDataClient for PontNetDataClient {
...
/// The PontNet-specific request context needed to update a peer's scoring.
struct PontNetResponseCallback {
    data_client: PontNetDataClient,
    id: ResponseId,
    peer: PeerNetworkId,
    request: StorageServiceRequest,
}

impl ResponseCallback for PontNetResponseCallback {
...
impl fmt::Debug for PontNetResponseCallback {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("PontNetResponseCallback")
            .field("data_client", &"..")
            .field("id", &self.id)
...
pub struct DataSummaryPoller {
    data_client: PontNetDataClient, // The data client through which to poll peers
...
impl DataSummaryPoller {
    ...
    pub async fn start_poller(self) {
        info!(
            (LogSchema::new(LogEntry::DataSummaryPoller).message("Starting the Pont data poller!"))
        );
