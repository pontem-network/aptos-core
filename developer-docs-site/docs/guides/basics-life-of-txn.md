...
For a deeper understanding of the lifecycle of an Pont transaction (from an operational perspective), we will follow a transaction on its journey, from being submitted to an Pont fullnode, to being committed to the Pont blockchain. We will then focus on the logical components of Pont nodes and take a look how the transaction interacts with these components.

## Assumptions

For the purpose of this doc, we will assume that:

* Alice and Bob are two users who each have an [account](/reference/glossary#account) on the Pont blockchain.
* Alice's account has 110 Pont Coins.
* Alice is sending 10 Pont Coins to Bob.
...
* An Pont client submits Alice's transaction to a REST service on an Pont Fullnode. The fullnode forwards this transaction to a validator fullnode which in turn forwards it to validator V<sub>1</sub>.
* Validator V<sub>1</sub> is a proposer/leader for the current round.

## Client submits a transaction

An Pont **client constructs a raw transaction** (let's call it Traw<sub>5</sub>) to transfer 10 Pont Coins from Alice’s account to Bob’s account. The Pont client signs the transaction with Alice's private key. The signed transaction T<sub>5</sub> includes the following:
...
| Move module | A module (or program) that indicates the actions to be performed on Alice's behalf. In this case, it contains:  <br />- A Move bytecode peer-to-peer [transaction script](/reference/glossary#transaction-script) <br />- A list of inputs to the script (for this example the list would contain Bob's account address and the payment amount in Pont Coins). |
...
| [Gas price](/reference/glossary#gas-price) | The amount (in Pont Coins) Alice is willing to pay per unit of gas, to execute the transaction. |
...
| [Chain ID](https://github.com/aptos-labs/pont-core/blob/main/types/src/chain_id.rs) | An identifier that distinguishes the Pont network deployments (to prevent cross-network attacks). |

## Lifecycle of the transaction

In this section, we will describe the lifecycle of transaction T<sub>5</sub>, from when the client submits it to when it is committed to the Pont blockchain.
...
We've described what happens in each stage below, along with links to the corresponding Pont node component interactions.

## Accepting the transaction

| Description                                                  | Pont Node Component Interactions                           |
| ------------------------------------------------------------ | ---------------------------------------------------------- |
| 1. **Client → REST service**: The client submits transaction T<sub>5</sub> to the REST service of an Pont fullnode. The fullnode uses the REST service to forward the transaction to its own mempool, which then forwards the transaction to mempools running on other nodes in the network. The transaction will eventually be forwarded to a mempool running on a validator Fullnode, which will send it to a validator node (V<sub>1</sub> in this case). | [1. REST Service](#1-client--rest-service)                  |
...
Alice's account will now have 100 Pont Coins, and its sequence number will be 6. If T<sub>5</sub> is replayed by Bob, it will be rejected as the sequence number of Alice's account (6) is greater than the sequence number of the replayed transaction (5).

## Pont node component interactions

In the [previous section](#lifecycle-of-the-transaction), we described the typical lifecycle of a transaction (from transaction submission to transaction commit). Now let's look at the inter-component interactions of Pont nodes as the blockchain processes transactions and responds to queries. This information will be most useful to those who:

* Would like to get an idea of how the system works under the covers.
* Are interested in eventually contributing to the Pont blockchain.

You can learn more about the different types of Pont nodes here:
...
The following are the core components of an Pont node used in the lifecycle of a transaction:
...
A client submits a transaction to the REST service of an Pont fullnode.
...
When a client performs a read query on the Pont blockchain (for example, to get the balance of Alice's account), the REST service interacts with the storage component directly to obtain the requested information.
...
* Note that just because a transaction T<sub>N</sub> was included in a proposed consensus block, it does not guarantee that T<sub>N </sub>will eventually be persisted in the distributed database of the Pont blockchain.
...
For implementation details refer to the [Mempool README](https://github.com/aptos-labs/pont-core/tree/main/documentation/specifications/mempool).
...
For implementation details refer to the [Consensus README](https://github.com/aptos-labs/pont-core/tree/main/documentation/specifications/consensus).
...
* Execution maintains a “scratchpad,” which holds in-memory copies of the relevant portions of the [Merkle accumulator](/reference/glossary#merkle-accumulator). This information is used to calculate the root hash of the current state of the Pont blockchain.
...
For implementation details refer to the [Execution README](https://github.com/aptos-labs/pont-core/tree/main/execution).
...
Note: The sequence number of an account on the Pont blockchain increments by one for each committed transaction originating from that account.
...
For implementation details refer to the [Storage README](https://github.com/aptos-labs/pont-core/tree/main/storage).
