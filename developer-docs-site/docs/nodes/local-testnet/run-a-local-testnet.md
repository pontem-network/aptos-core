...
You can run a local testnet of the Pont blockchain. This local testnet will not be connected to the Pont devnet. It will run on your local machine, independent of other Pont networks. You can use this local testnet for testing and development purposes.

You can run a local testnet in two ways:

1. Using the Pont-core source code. This approach is useful for testing modifications to the Pont-core codebase or to the Pont Framework.

2. Using Docker. This is particularly useful for building services on top of the Pont blockchain or the Pont Framework, as there is no build overhead and the ledger persists across network restarts (by default).
...
## Using the Pont-core source code
...
5. With your development environment ready, now you can start your testnet network. Before you proceed, make a note of the following:
    ...
    :::tip
     - When you run the below command to start the local testnet, your terminal will enter into an interactive mode, with a message `Pont is running, press ctrl-c to exit`. Hence, you will need to open another shell terminal for the subsequent steps described in this section.
    ...
    ```
    CARGO_NET_GIT_FETCH_WITH_CLI=true cargo run -p pont-node -- --test
    ```
    ...
    Completed generating configuration:
        ...
        Pont root key path: "/private/var/folders/gn/m74t8ylx55z935q8wx035qn80000gn/T/b3adc18c144bfcc78a1541953893bc1c/mint.key"
...
**NOTE**: The above command starts a local testnet with a single validator node. The command runs `pont-node` from a genesis-only ledger state. If you want to reuse the ledger state produced by a previous run of `pont-node`, then use:

```
cargo run -p pont-node -- --test --config <config-path>
```

### Attaching a Faucet to your testnet

Faucets are stateless services that can be run in parallel with the testnet. A Faucet is a way to create Pont test coins with no real-world value. You can use the Faucet by sending a request to create coins and transfer them into a given account on your behalf.

1. Make sure that you started your local testnet as described in Step 5 above.
2. Open a new shell terminal.
3. Copy the _Pont root key path_ from your terminal where you started the testnet, and use it to replace the `mint-key-file-path` in the below command.
4. Run the following command to start a Faucet:
```
   cargo run --package pont-faucet -- \
      --chain-id TESTING \
...
3. Download the YAML configuration files for:
  ...
  - [Validator testnet docker compose](https://github.com/aptos-labs/pont-core/blob/main/docker/compose/validator-testnet/docker-compose.yaml) and
  - [Validator configuration](https://github.com/aptos-labs/pont-core/blob/main/docker/compose/validator-testnet/validator_node_template.yaml).
...
```bash
mkdir pont_local_validator && cd pont_local_validator
wget https://raw.githubusercontent.com/aptos-labs/pont-core/main/docker/compose/validator-testnet/docker-compose.yaml
wget https://raw.githubusercontent.com/aptos-labs/pont-core/main/docker/compose/validator-testnet/validator_node_template.yaml
docker-compose up
```
...
Completed generating configuration:
        ...
        Pont root key path: "/tmp/694173aa3bbe019499bbd5cf3fe0e2fc/mint.key"
...
Use the [Pont CLI tool](https://github.com/aptos-labs/pont-core/blob/main/crates/pont/README.md) to interact with your local testnet. The above output contains information you will use for starting the [Pont CLI tool](https://github.com/aptos-labs/pont-core/blob/main/crates/pont/README.md):

* `Pont root key path`: The root key (also known as the mint or faucet key) controls the account that can mint tokens. Available in the docker compose folder under `pont_root_key`.
...
* [Interacting with the Pont Blockchain](/guides/interacting-with-the-pont-blockchain) to learn how to mint coins.
