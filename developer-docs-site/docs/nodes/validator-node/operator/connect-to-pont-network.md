...
7. Download the `genesis.blob` and `waypoint.txt` files published by Pont Labs team. 
   - See [Node Files](/nodes/node-files) for locations and commands to download these files.
...
See [node liveness defined here](https://pont.dev/reference/node-liveness-criteria/#verifying-the-liveness-of-your-node). 
:::
