...
The below CLI command examples use mainnet. See the `--rest-url` value for testnet or devnet in [Pont Blockchain Deployments](/docs/nodes/pont-deployments.md).
:::

### 1. Initialize Pont CLI

  ```bash
  pont init --profile mainnet-operator \
...
Make sure you have enough APT coins to pay for gas. You can check for this either on the Pont Explorer or using the CLI:

- On the Pont Explorer `https://explorer.pontlabs.com/account/<account-address>?network=Mainnet`, or 
- Use the CLI:

  ```bash
  pont account list --profile mainnet-operator
  ```
...
```bash
pont node update-validator-network-addresses  \
...
```bash
pont node update-consensus-key  \
  --pool-address <pool-address> \
  --operator-config-file ~/$WORKSPACE/$USERNAME/operator.yaml \
  --profile mainnet-operator
```
...
Run the command `pont node get-stake-pool` as shown in [Checking your stake pool information](#checking-your-stake-pool-information). You can also follow these steps: [How to find out when the next epoch starts](/issues-and-workarounds#how-to-find-out-when-the-next-epoch-starts).
:::

### 6. Check the validator set
   
When you join the validator set, your validator node will be in "Pending Active" state until the next epoch occurs. **During this time you might see errors like "No connected PontNet peers". This is normal.** Run the below command to look for your validator in the "pending_active" list.

```bash
pont node show-validator-set --profile mainnet-operator | jq -r '.Result.pending_active' | grep <pool_address>
```
...
```bash
pont node show-validator-set --profile mainnet-operator | jq -r '.Result.active_validators' | grep <pool_address>
```

## Checking your stake pool information

:::tip How validation works
Before you proceed, see [Validation on the Pont blockchain](/concepts/staking#validation-on-the-pont-blockchain) for a brief overview.
:::
...
:::tip
For testnet or devnet `--url` field values, see [Pont Blockchain Deployments](/nodes/pont-deployments).
:::
...
- This can be set only by the stake pool owner. Operator receives the "commission_percentage" of the generated staking rewards. If you request the commission (you can do so by running the command `pont stake request-commission`), then at the end of the `lockup_expiration_utc_time` the commission part of the rewards will go to the operator address while the rest will stay in the stake pool and belong to the owner. Here "the commission part of the rewards" means the value of **commission_not_yet_unlocked**. 

  For example, in a scenario with a lock up of one month, you call `pont stake request-commission` every month. This will pay out the commission that was accrued during the previous month but only when unlocked at the end of the previous month. Regardless of how often you run `pont stake request-commission` during the month, the commission is only paid out upon the completion of `lockup_expiration_utc_time`.
...
- The date when the commission will unlock. However, this unlocked commission will not be auto-disbursed. It will only disburse when the command `pont stake request-commission` is called again.
...
Either an owner or an operator can request commission. You can request commission at the end of a lockup period, i.e., at the end of **lockup_expiration_utc_time**, by running the `pont stake request-commission` command. Make sure to provide the operator and the owner addresses. See an example command below:

```bash
pont stake request-commission \
...
- Successful leader-validator proposals during the current epoch. Also see [Validation on the Pont blockchain](/concepts/staking#validation-on-the-pont-blockchain) for the distinction between leader-validator and the voter-validator.
...
```bash
pont node analyze-validator-performance \
