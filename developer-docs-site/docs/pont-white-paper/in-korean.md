---
title: "한글 (In Korean)"
slug: "pont-white-paper-in-korean"
---
...
된 Pont 블록체인을 제시한다. 지난 3년간 전 세계 350명 이상의 개발자들이 Pont 블록체인을
개발했다 [1]. Pont는 합의, 스마트 컨트랙트 설계, 시스템 보안, 성능 및 탈중앙성 면에서 새롭고
참신한 혁신을 제안한다. 이러한 기술들의 조합은 웹3 대중화를 위한 기본 구성 요소가 될 것이다:

* 먼저, Pont 블록체인은 빠르고 안전한 트랜잭션 실행을 위해 Move 언어를 자체적으로 통합
...
* 둘째, Pont 데이터 모델은 유연한 키 관리 및 하이브리드 수탁 옵션을 지원한다. 이는 서명
전 트랜잭션 투명성과 함께 실제 라이트 클라이언트 프로토콜과 함께 보다 안전하고 신뢰할
수 있는 사용자 경험을 제공한다.
* 셋째, 높은 처리량과 낮은 지연 시간을 달성하기 위해 Pont 블록체인은 트랜잭션 처리의 주
...
른 병렬 실행 엔진과 달리 Pont 블록체인은 개발자에게 그러한 제한을 두지 않는다. 임의로
복잡한 트랜잭션이 원자성을 효율적으로 지원하여 실제 애플리케이션의 처리량을 높이고 대
기 시간을 단축할 수 있으며 개발을 단순화할 수 있다.
* 다섯째, Pont의 모듈형으로 설계된 아키텍처는 클라이언트 유연성을 지원하고 빈번하고 즉
각적인 업그레이드를 위해 최적화되었다. 또한 Pont 블록체인은 내장된 온체인 변경 관리
...
- **초록**: Pont 백서의 한국어 버전 전체 PDF를 보려면 [여기를 클릭하십시오](/papers/whitepaper-korean.pdf).
