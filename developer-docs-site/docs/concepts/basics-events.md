...
Events are emitted during the execution of a transaction. Each Move module can define its own events and choose when to emit the events upon execution of the module. For example, during a [coin transfer](https://pont.dev/tutorials/your-first-transaction/), both the sender and receiver's accounts will emit `SentEvent` and `ReceivedEvent`, respectively. This data is stored within the ledger and can be queried via the REST interface's [Get events by event handle](https://fullnode.devnet.pontlabs.com/v1/spec#/operations/get_events_by_event_handle).

Assuming that an account `0xc40f1c9b9fdc204cf77f68c9bb7029b0abbe8ad9e5561f7794964076a4fbdcfd` had sent coins to another account, the following query could be made to the REST interface: `https://fullnode.devnet.pontlabs.com/v1/accounts/c40f1c9b9fdc204cf77f68c9bb7029b0abbe8ad9e5561f7794964076a4fbdcfd/events/0x1::coin::CoinStore<0x1::pont_coin::PontCoin>/withdraw_events`. The output would be all `WithdrawEvent`s stored on that account, it would look like 
...
Each registered event has a unique `key`. The key `0x0000000000000000c40f1c9b9fdc204cf77f68c9bb7029b0abbe8ad9e5561f7794964076a4fbdcfd` maps to the event `0x1::coin::CoinStore<0x1::pont_coin::PontCoin>/sent_events` registered on account `0xc40f1c9b9fdc204cf77f68c9bb7029b0abbe8ad9e5561f7794964076a4fbdcfd`. This key can then be used to directly make event queries, e.g., `https://fullnode.devnet.pontlabs.com/v1/events/0000000000000000c40f1c9b9fdc204cf77f68c9bb7029b0abbe8ad9e5561f7794964076a4fbdcfd`.
...
[coin_transfer]: https://github.com/aptos-labs/pont-core/blob/bdd0a7fe82cd6aab4b47250e5eb6298986777cf7/pont-move/framework/pont-framework/sources/coin.move#L412
[get_events]: https://fullnode.devnet.pontlabs.com/v1/spec#/operations/get_events_by_event_handle
