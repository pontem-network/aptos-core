---
title: "Using Pont CLI"
id: "use-pont-cli"
---

# Using Pont CLI

The `pont` tool is a command line interface (CLI) for developing on the Pont blockchain, debugging, and for node operations. This document describes how to use the `pont` CLI tool. To install the CLI, see [Install Pont CLI](install-pont-cli).

## Command line help

Command line help is available.  Type `pont help` or `pont --help` to see the available command options. See below the usage output from `pont --help`:
...
SUBCOMMANDS:
    ...
    config        Tool for interacting with configuration of the Pont CLI tool
    genesis       Tool for setting up an Pont chain Genesis transaction
    governance    Tool for on-chain governance
    help          Print this message or the help of the given subcommand(s)
    info          Show build information about the CLI
    init          Tool to initialize current directory for the pont tool
...
Command-specific help is also available.  For example, see below the usage output from `pont move --help`:

```bash

USAGE:
    pont move <SUBCOMMAND>
...
SUBCOMMANDS:
    ...
    publish
            Publishes the modules in a Move package to the Pont blockchain
    run
            Run a Move function
    test
...
Help for sub-commands is also available.  For example, see below the usage output from `pont move compile --help`:

```bash

USAGE:
    pont move compile [OPTIONS]
...
Run the `pont info` command to get the CLI information for debugging purposes. See an example output of the `pont info` command:
...
1. Your configurations are in a **local** YAML configuration file `.pont/config.yaml`, i.e., located in the current working directory where you run the CLI. In this case you must run your CLI commands from this current working directory for this configuration to be used.
2. You can verify that the CLI is set to use this local configuration YAML file by running the command:

  ```bash
  pont config show-global-config
  ```
  ...
  ```
  The `Workspace` value for the `config_type` indicates that the `.pont/config.yaml` file is used for the CLI configuration.

### In the home directory for the global runs

1. Your configurations are in a **global** YAML configuration file `~/.pont/global_config.yaml`, i.e., located in your home directory.
2. Set the CLI to use this global configuration YAML file by running this command:
  ```bash
  pont config set-global-config --config-type global
  ```
...
If you did not set any global configuration, then the `./.pont/config.yaml` in the current working directory is used for configuration.
:::
...
```bash
pont config generate-shell-completions --shell zsh --output-file ~/.oh-my-zsh/completions/_pont
```

## Initialize local configuration and create an account

A local folder named `.pont/` will be created with a configuration `config.yaml` which can be used to store configuration between CLI runs.  This is local to your run, so you will need to continue running CLI from this folder, or reinitialize in another folder.

### Step 1: Run Pont init

The `pont init` command will initialize the configuration with the private key you provided.

```bash
$ pont init
Configuring for profile default
Enter your rest endpoint [Current: None | No input: https://fullnode.devnet.pontlabs.com]

No rest url given, using https://fullnode.devnet.pontlabs.com...
Enter your faucet endpoint [Current: None | No input: https://faucet.devnet.pontlabs.com]
...
Pont is now set up for account 00f1f20ddd0b0dd2291b6e42c97274668c479bca70f07c6b6a80b99720779696!  Run `pont help` for more information about commands
...
To change the configuration, you can either run the command `pont init` or you can manually edit the `.pont/config.yaml` that is in your current working directory.
...
Pont is now set up for account 18B61497FD290B02BB0751F44381CADA1657C2B3AA6194A00D9BC9A85FAD3B04!  Run `pont help` for more information about commands
{
  "Result": "Success"
}
```
...
You can fund an account with the faucet via the CLI by using either an account address or with `default` (which defaults to the account address created with `pont init`).

For example, to fund the account `00f1f20ddd0b0dd2291b6e42c97274668c479bca70f07c6b6a80b99720779696` that was created above with the `pont init` command:

```bash
$ pont account fund-with-faucet --account 00f1f20ddd0b0dd2291b6e42c97274668c479bca70f07c6b6a80b99720779696
{
...
```bash
$ pont account list --query balance --account 00f1f20ddd0b0dd2291b6e42c97274668c479bca70f07c6b6a80b99720779696
```
or by specifying the `default` as below:
```bash
$ pont account list --query balance --account default
```
...
```bash
$ pont account list --query resources --account default
```
or
```bash
$ pont account list --query resources --account 0xf1f20ddd0b0dd2291b6e42c97274668c479bca70f07c6b6a80b99720779696
```
...
```bash
$ pont account list --query resources --account superuser
{
  "Result": [
    {
      "coin": {
...
```bash
$ pont account list --query modules
{
  "Result": [
    {
...
The Pont CLI is a simple wallet as well, and can transfer coins between accounts.
```bash
$ pont account transfer --account superuser --amount 100
{
  "Result": {
    "gas_used": 73,
    "balance_changes": {
...
To allow generating private keys, you can use the `pont key generate command`.  You can generate
either `x25519` or `ed25519` keys.
```bash
$ pont key generate --key-type ed25519 --output-file output.key
{
  "Result": {
...
the `pont` CLI to generate a peer configuration and write it into a file named `peer_config.yaml`.
```bash
$ pont key extract-peer --output-file peer_config.yaml
```
...
The `pont` CLI can be used to compile a Move package locally.
The below example uses the `HelloBlockchain` in [move-examples](https://github.com/aptos-labs/pont-core/tree/main/pont-move/move-examples).

The named addresses can be either an account address, or a profile name.

```bash
$ pont move compile --package-dir pont-move/move-examples/hello_blockchain/ --named-addresses hello_blockchain=superuser
```
...
The `pont` CLI can also be used to compile and run unit tests locally.
In this example, we'll use the `HelloBlockchain` in [move-examples](https://github.com/aptos-labs/pont-core/tree/main/pont-move/move-examples).

```bash
$ pont move test --package-dir pont-move/move-examples/hello_blockchain/ --named-addresses hello_blockchain=superuser
```
The above command will generate the following terminal output:
```bash
INCLUDING DEPENDENCY PontFramework
INCLUDING DEPENDENCY PontStdlib
...
The `pont` CLI can be used to run [Move Prover](https://github.com/move-language/move/tree/main/language/move-prover) which is a formal verification tool for the Move language. The below example proves the `hello_prover` package in [move-examples](https://github.com/aptos-labs/pont-core/tree/main/pont-move/move-examples).
```bash
...
```
In this case, see [Install the dependencies of Move Prover](install-pont-cli#step-3-optional-install-the-dependencies-of-move-prover).

### Debug and print stack trace

In this example, we will use `DebugDemo` in [debug-move-example](https://github.com/aptos-labs/pont-core/tree/main/crates/pont/debug-move-example).

First, you need to include Move nursery in your Move [toml file](https://github.com/aptos-labs/pont-core/tree/main/crates/pont/debug-move-example/Move.toml).

Now, you can use `Debug::print` and `Debug::print_stack_trace` in your [DebugDemo Move file](https://github.com/aptos-labs/pont-core/tree/main/crates/pont/debug-move-example/sources/DebugDemo.move).

You can run the following command:
```bash
$ pont move test --package-dir crates/pont/debug-move-example
```
...
```bash
$ pont move publish --package-dir pont-move/move-examples/hello_blockchain/ --named-addresses HelloBlockchain=8946741e5c907c43c9e042b3739993f32904723f8e2d1491564d38959b59ac71
```

You can additionally use named profiles for the addresses.  The first placeholder is `default`
```bash
$ pont move publish --package-dir pont-move/move-examples/hello_blockchain/ --named-addresses HelloBlockchain=default
```
...
```bash
$ pont move run --function-id 0xb9bd2cfa58ca29bce1d7add25fce5c62220604cd0236fe3f90d9de91ed9fb8cb::message::set_message --args string:hello!
{
  "Result": {
    "changes": [
      {
      ...
      },
      {
        ...
        "event": "write_resource",
        "resource": "0x1::coin::CoinStore<0x1::pont_coin::PontCoin>"
      },
      {
        "address": "b9bd2cfa58ca29bce1d7add25fce5c62220604cd0236fe3f90d9de91ed9fb8cb",
        "data": {
...
```bash
$ pont move run --function-id default::message::set_message --args string:hello!
{
  "Result": {
    "changes": [
      {
...
You can run a local testnet from the pont CLI, that will match the version it was built with.  Additionally, it can
...
The `pont` tool supports bootstrapping new blockchains through what is known as a genesis ceremony. The output of the genesis ceremony is the output of move instructions that prepares a blockchain for online operation. The input consists of:
...
* For test chains, there also exists an account that manages the minting of PontCoin

## Generating genesis

* The genesis organizer constructs a `Layout` and distributes it.
* The genesis organizer prepares the Pont framework's bytecode and distributes it.
...
* Each participant begins their `pont-node`. The `pont-node` verifies upon startup that the `genesis.blob` with the waypoint provided by the genesis organizer .
* The blockchain will begin consensus after a quorum of stake is available.

### Prepare pont-core

The following guide assumes that you have access to the Pont-core repository or the associated tools. You can download and prepare Pont-core from [GitHub](https://github.com/aptos-labs/pont-core):
...
* `root_key`: an Ed25519 public key for PontCoin management.
* `users`: the set of participants
* `chain_id`: the `ChainId` or a unique integer that distinguishes this deployment from other Pont networks
...
### Building the Pont Framework

From your Pont-core repository, build the framework and package it:
```
cargo run --package framework
mkdir pont-framework-release
cp pont-framework/releases/artifacts/current/build/**/bytecode_modules/* pont-framework-release
```

The framework will be stored within the `pont-framework-release` directory.
...
To generate this using the `pont` CLI:

1. Generate your validator's keys:

```
cargo run --package pont -- genesis generate-keys --output-dir bobs
```

2. Generate your `ValidatorConfiguration`:

```
cargo run --package pont -- \\
...
* Use the `pont` CLI to generate genesis and waypoint:

```
cargo run --package pont -- genesis generate-genesis --local-repository-dir genesis
```

### Starting an `pont-node`
