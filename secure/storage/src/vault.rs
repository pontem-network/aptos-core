...
use pont_vault_client::ReadResponse;
...
impl VaultStorage {
        ...
        if self.renew_ttl_secs.is_some() {
            ...
            if now >= next_renewal {
                ...
                if let Ok(ttl) = result {
                    ...
                    pont_logger::error!("Unable to renew lease: {}", e.to_string());
                }
            }
        }
        &self.client
    ...
    fn reset_crypto(&self) -> Result<(), Error> {
        let keys = match self.client().list_keys() {
            ...
            Err(pont_vault_client::Error::NotFound(_, _)) => return Ok(()),
...
impl CryptoStorage for VaultStorage {
    ...
    fn sign<T: CryptoHash + Serialize>(
        ...
        let name = self.crypto_name(name);
        let mut bytes = <T::Hasher as pont_crypto::hash::CryptoHasher>::seed().to_vec();
...
pub mod policy {
    ...
    use pont_vault_client as vault;

    const APTOS_DEFAULT: &str = "pont_default";
