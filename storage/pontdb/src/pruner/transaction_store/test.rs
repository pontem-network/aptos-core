...
use crate::{LedgerPrunerManager, LedgerStore, PontDB, PrunerManager, TransactionStore};
...
use pont_config::config::LedgerPrunerConfig;
use pont_types::{
...
fn verify_write_set_pruner(write_sets: Vec<WriteSet>) {
    ...
    let transaction_store = &pont_db.transaction_store;
    let num_write_sets = write_sets.len();

    let pruner = LedgerPrunerManager::new(
        Arc::clone(&pont_db.ledger_db),
        Arc::clone(&pont_db.state_store),
        LedgerPrunerConfig {
            enable: true,
            prune_window: 0,
            batch_size: 1,
    ...
    pont_db.ledger_db.write_schemas(batch).unwrap();
...
fn verify_txn_store_pruner(
    ...
    let pont_db = PontDB::new_for_test(&tmp_dir);
    let transaction_store = &pont_db.transaction_store;
    let ledger_store = LedgerStore::new(Arc::clone(&pont_db.ledger_db));
    ...
    for i in (0..=num_transaction).step_by(step_size) {
        ...
        for j in i..num_transaction {
            ...
            pont_db.get_accumulator_summary(j as Version).unwrap();
        }
