...
use pont_config::config::{LedgerPrunerConfig, StateMerklePrunerConfig};
...
use pont_types::state_store::state_value::StaleStateValueIndex;
use pont_types::{
...
use crate::{
    ...
    LedgerPrunerManager, PontDB, PrunerManager, StatePrunerManager,
};
...
fn test_state_store_pruner() {
    ...
    let pont_db = PontDB::new_for_test_no_cache(&tmp_dir);
    let state_store = &pont_db.state_store;
    ...
    {
        let pruner = create_state_pruner_manager(&pont_db.state_merkle_db, prune_batch_size);
...
fn test_state_store_pruner_disabled() {
    ...
    let num_versions = 25;
    let tmp_dir = TempPath::new();
    let pont_db = PontDB::new_for_test(&tmp_dir);
    let state_store = &pont_db.state_store;
...
fn test_worker_quit_eagerly() {
    ...
    let tmp_dir = TempPath::new();
    let pont_db = PontDB::new_for_test(&tmp_dir);
    let db = Arc::clone(&pont_db.ledger_db);
    let state_store = &pont_db.state_store;
...
fn verify_state_value_pruner(inputs: Vec<Vec<(StateKey, Option<StateValue>)>>) {
    let tmp_dir = TempPath::new();
    let db = PontDB::new_for_test(&tmp_dir);
