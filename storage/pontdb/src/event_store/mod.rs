...
impl EventStore {
    ...
    pub fn get_event_by_version_and_index(
        ...
        self.db
            ...
            .ok_or_else(|| {
                PontDbError::NotFound(format!("Event {} of Txn {}", index, version)).into()
            })
    }
    ...
    fn lookup_event_by_key(
        ...
        if indices.is_empty() {
            return Err(PontDbError::NotFound(format!(
                "Event {} of seq num {}.",
                event_key, seq_num
            ))
            .into());
    ...
    pub fn get_block_metadata(&self, version: Version) -> Result<(Version, NewBlockEvent)> {
        let (first_version, event_index, seq_num) = self
            ...
            .ok_or_else(|| PontDbError::NotFound("NewBlockEvent".to_string()))?;
