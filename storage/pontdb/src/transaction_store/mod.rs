...
impl TransactionStore {
    ...
    pub fn get_transaction(&self, version: Version) -> Result<Transaction> {
        self.db
            ...
            .ok_or_else(|| PontDbError::NotFound(format!("Txn {}", version)).into())
    }
    ...
    pub fn get_write_set(&self, version: Version) -> Result<WriteSet> {
        self.db
            ...
            .ok_or_else(|| PontDbError::NotFound(format!("WriteSet at version {}", version)).into())
    }
    ...
    /// More details are in this issue https://github.com/aptos-labs/pont-core/issues/1288.
