...
//! This crate provides [`PontDB`] which represents physical storage of the core Pont data
//! structures.
//!
//! It relays read/write operations on the physical storage via [`schemadb`] to the underlying
//! Key-Value storage system, and implements pont data structures on top of it.
...
#[cfg(any(test, feature = "fuzzing"))]
use pont_config::config::DEFAULT_MAX_NUM_NODES_PER_LRU_CACHE_SHARD;
...
use pont_types::proof::TransactionAccumulatorSummary;
...
static ROCKSDB_PROPERTY_MAP: Lazy<HashMap<&str, String>> = Lazy::new(|| {
    ...
    .iter()
    .map(|x| (*x, format!("pont_{}", x.replace('.', "_"))))
    .collect()
});

fn error_if_too_many_requested(num_requested: u64, max_allowed: u64) -> Result<()> {
    if num_requested > max_allowed {
        Err(PontDbError::TooManyRequested(num_requested, max_allowed).into())
    } else {
        Ok(())
    }
}
...
fn update_rocksdb_properties(ledger_rocksdb: &DB, state_merkle_rocksdb: &DB) -> Result<()> {
    ...
    for cf_name in ledger_db_column_families() {
        for (rockdb_property_name, pont_rocksdb_property_name) in &*ROCKSDB_PROPERTY_MAP {
            ROCKSDB_PROPERTIES
                .with_label_values(&[cf_name, pont_rocksdb_property_name])
...
/// access to the core Pont data structures.
#[derive(Debug)]
pub struct PontDB {
    ledger_db: Arc<DB>,
...
impl PontDB {
    ...
    pub fn open<P: AsRef<Path> + Clone>(
        ...
        info!(
            ...
            "Opened PontDB (LedgerDB + StateMerkleDB).",
        );
        Ok(myself)
    }
    ...
    fn open_indexer(
        ...
        if indexer.next_version() < ledger_next_version {
            ...
            while next_version < ledger_next_version {
                info!(next_version = next_version, "PontDB Indexer catching up. ",);
...
impl DbReader for PontDB {
    ...
    /// Returns the transaction by version, delegates to `PontDB::get_transaction_with_proof`.
    ...
    fn get_table_info(&self, handle: TableHandle) -> Result<TableInfo> {
        gauged_api("get_table_info", || {
            self.get_table_info_option(handle)?
                .ok_or_else(|| PontDbError::NotFound(format!("TableInfo for {:?}", handle)).into())
        })
    }
...
impl GetRestoreHandler for Arc<PontDB> {
    fn get_restore_handler(&self) -> RestoreHandler {
        RestoreHandler::new(
