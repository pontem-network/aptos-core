...
use crate::{
    ...
    LedgerStore, PontDbError, StaleNodeIndexCrossEpochSchema, StaleNodeIndexSchema,
...
use pont_jellyfish_merkle::iterator::JellyfishMerkleIterator;
    ...
    fn get_state_storage_usage(&self, version: Option<Version>) -> Result<StateStorageUsage> {
        version.map_or(Ok(StateStorageUsage::zero()), |version| {
            Ok(self
                ...
                .ok_or_else(|| PontDbError::NotFound(format!("VersionData at {}", version)))?
...
impl StateStore {
    ...
    pub fn get_values_by_key_prefix(
        ...
        // pont/abc|1
        // pont/abd|1
        // if we seek('pont/'), and call next, we may not reach `pont/abd/1` because the prefix extractor we adopted
        // here will stick with prefix `pont/abc` and return `None` or any arbitrary result after visited all the
        // keys starting with `pont/abc`.
    ...
    pub fn merklize_value_set(
        ...
        node_hashes: Option<&HashMap<pont_types::nibble::nibble_path::NibblePath, HashValue>>,
        version: Version,
    ...
    pub fn get_value_chunk_with_proof(
        ...
        let state_key_values: Vec<(StateKey, StateValue)> = result_iter
            ...
            PontDbError::NotFound(format!("State chunk starting at {}", first_index)),
        );
    ...
    // TODO: change to async comment once it does like https://github.com/aptos-labs/pont-core/blob/159b00f3d53e4327523052c1b99dd9889bf13b03/storage/backup/backup-cli/src/backup_types/state_snapshot/restore.rs#L147 or overlap at least two chunks.
    ...
    pub fn get_all_jmt_nodes_referenced(
        &self,
        version: Version,
    ) -> Result<Vec<pont_jellyfish_merkle::node_type::NodeKey>> {
        pont_jellyfish_merkle::JellyfishMerkleTree::new(self.state_merkle_db.as_ref())
            .get_all_nodes_referenced(version)
    }

    #[cfg(test)]
    pub fn get_all_jmt_nodes(&self) -> Result<Vec<pont_jellyfish_merkle::node_type::NodeKey>> {
        let mut iter = self
            .state_db
            .state_merkle_db
            .db
