...
use pont_api::bootstrap as bootstrap_api;
use pont_build_info::build_information;
use pont_config::{
    config::{
        BaseConfig, DataStreamingServiceConfig, NetworkConfig, NodeConfig, PersistableConfig,
        PontDataClientConfig, StorageServiceConfig,
    },
    network_id::NetworkId,
    utils::get_genesis_txn,
};
use pont_data_client::pontnet::PontNetDataClient;
use pont_fh_stream::runtime::bootstrap as bootstrap_fh_stream;
use pont_infallible::RwLock;
use pont_logger::{prelude::*, telemetry_log_writer::TelemetryLog, Level, LoggerFilterUpdater};
...
/// Runs an pont fullnode or validator
#[derive(Clone, Debug, Parser)]
#[clap(name = "Pont Node", author, version)]
pub struct PontNodeArgs {
    ...
    /// Paths to the Pont framework release package to be used for genesis.
    #[clap(long, requires("test"))]
...
pub fn start(
    ...
    let mut logger_builder = pont_logger::Logger::builder();
    logger_builder
    ...
    let logger_filter_update_job = pont_logger::LoggerFilterUpdater::new(logger, logger_builder);

    // Print out build information.
    log_build_information();

    // Let's now log some important information, since the logger is set up
    info!(config = config, "Loaded PontNode config");
    ...
    R: ::rand::RngCore + ::rand::CryptoRng,
{
    ...
    let test_dir = if let Some(test_dir) = test_dir {
        ...
        pont_temppath::TempPath::new().as_ref().to_path_buf()
    };
    ...
    let pont_root_key_path = test_dir.join("mint.key");
    ...
    let config = if validator_config_path.exists() {
        ...
        let builder = pont_genesis::builder::Builder::new(&test_dir, framework.clone())?
        ...
        let mut key_file = std::fs::File::create(&pont_root_key_path)?;
    ...
    println!("\tPont root key path: {:?}", pont_root_key_path);
    ...
    println!(
        "\tPontnet Fullnode network endpoint: {}",
...
fn fetch_chain_id(db: &DbReaderWriter) -> anyhow::Result<ChainId> {
    let db_state_view = db
        ...
        .map_err(|err| anyhow!("[pont-node] failed to create db state view {}", err))?;
    Ok(db_state_view
        .as_account_with_state_view(&CORE_CODE_ADDRESS)
        .get_chain_id_resource()
        .map_err(|err| anyhow!("[pont-node] failed to get chain id resource {}", err))?
        .expect("[pont-node] missing chain ID resource")
        .chain_id())
}
...
fn create_state_sync_runtimes<M: MempoolNotificationSender + 'static>(
    ...
    let (pont_data_client, pont_data_client_runtime) = setup_pont_data_client(
        node_config.state_sync.storage_service,
...
fn setup_data_streaming_service(
    ...
    let data_streaming_service =
        DataStreamingService::new(config, pont_data_client, streaming_service_listener);
...
fn setup_pont_data_client(
    storage_service_config: StorageServiceConfig,
    pont_data_client_config: PontDataClientConfig,
    base_config: BaseConfig,
    network_handles: HashMap<NetworkId, storage_service_client::StorageServiceNetworkSender>,
    peer_metadata_storage: Arc<PeerMetadataStorage>,
) -> anyhow::Result<(PontNetDataClient, Runtime)> {
    ...
    let pont_data_client_runtime = Builder::new_multi_thread()
        .thread_name_fn(|| {
            static ATOMIC_ID: AtomicUsize = AtomicUsize::new(0);
            let id = ATOMIC_ID.fetch_add(1, Ordering::SeqCst);
        ...
        .enable_all()
        .build()
        .map_err(|err| anyhow!("Failed to create pont data client {}", err))?;

    // Create the data client and spawn the data poller
    let (pont_data_client, data_summary_poller) = PontNetDataClient::new(
        pont_data_client_config,
        base_config,
        storage_service_config,
        TimeService::real(),
        network_client,
        Some(pont_data_client_runtime.handle().clone()),
    );
    pont_data_client_runtime.spawn(data_summary_poller.start_poller());

    Ok((pont_data_client, pont_data_client_runtime))
}
...
fn bootstrap_indexer(
    ...
    use pont_indexer::runtime::bootstrap as bootstrap_indexer_stream;

    match bootstrap_indexer_stream(&node_config, chain_id, pont_db, mp_client_sender) {
        None => Ok(None),
...
pub fn setup_environment(
    ...
    let (pont_db, db_rw) = DbReaderWriter::wrap(
        PontDB::open(
    ...
    {
        PontVM::set_processed_transactions_detailed_counters();
    }
    ...
    let telemetry_runtime = pont_telemetry::service::start_telemetry_service(
        node_config.clone(),
        chain_id,
        build_info,
        remote_log_rx,
    ...
    for network_config in network_configs.into_iter() {
        ...
        let (mempool_sender, mempool_events) = network_builder.add_p2p_service(
            &pont_mempool::network::network_endpoint_config(MEMPOOL_NETWORK_CHANNEL_BUFFER_SIZE),
        );
    ...
    let index_runtime = bootstrap_indexer(&node_config, chain_id, pont_db, mp_client_sender)?;
    ...
    instant = Instant::now();
    let mempool = pont_mempool::bootstrap(
        &node_config,
...
mod tests {
    ...
    use pont_config::config::{NodeConfig, WaypointConfig};
    ...
    #[test]
    fn test_pont_vm_does_not_have_test_natives() {
