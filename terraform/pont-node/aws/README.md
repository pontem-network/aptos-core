...
Install pre-requisites if needed:
   ...
   * Pont CLI: https://github.com/aptos-labs/pont-core/releases/tag/pont-cli-v0.1.0-alpha
...
4. Modify `main.tf` file to configure Terraform, and create fullnode from Terraform module. Example content for `main.tf`:
  ...
  module "pont-node" {
    ...
    source        = "github.com/aptos-labs/pont-core.git//terraform/pont-node/aws?ref=main"
...
For the full customization options, see the variables file [here](https://github.com/aptos-labs/pont-core/blob/main/terraform/pont-node/aws/variables.tf), and the [helm values](https://github.com/aptos-labs/pont-core/blob/main/terraform/helm/pont-node/values.yaml).
...
9. Get your node IP info:
    ...
    ```
    $ export VALIDATOR_ADDRESS="$(kubectl get svc ${WORKSPACE}-pont-node-validator-lb --output jsonpath='{.status.loadBalancer.ingress[0].hostname}')"

    $ export FULLNODE_ADDRESS="$(kubectl get svc ${WORKSPACE}-pont-node-fullnode-lb --output jsonpath='{.status.loadBalancer.ingress[0].hostname}')"
    ```
...
11. Configure validator information.
    ...
    This will create a YAML file in your working directory with your username, e.g. `pontbot.yaml`, it should looks like below:
...
13. Download PontFramework Move bytecodes into a folder named `framework`.
