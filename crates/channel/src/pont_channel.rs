...
//! pont_channel provides an mpsc channel which has two ends `pont_channel::Receiver`
//! and `pont_channel::Sender` similar to existing mpsc data structures.
...
use pont_infallible::{Mutex, NonZeroUsize};
use pont_metrics_core::IntCounterVec;
use std::{
    fmt::{Debug, Formatter},
    hash::Hash,
    pin::Pin,
...
/// The sending end of the pont_channel.
#[derive(Debug)]
pub struct Sender<K: Eq + Hash + Clone, M> {
    shared_state: Arc<Mutex<SharedState<K, M>>>,
}

/// The status of an element inserted into a pont_channel. If the element is successfully
...
/// The receiving end of the pont_channel.
#[derive(Debug)]
...
/// Configuration for a new pont_channel queue.
#[derive(Clone, Copy)]
pub struct Config {
...
impl Config {
    /// The pont_channel has a "sub-queue" per key. The `max_capacity` controls
...
pub fn new<K: Eq + Hash + Clone, M>(
    ...
    let max_queue_size_per_key =
        NonZeroUsize!(max_queue_size_per_key, "pont_channel cannot be of size 0");
