...
use pont_config::config::{Peer, PeerRole, PeerSet};
use pont_infallible::RwLock;
use pont_rest_client::{error::RestError, Response};
use pont_types::{
...
impl PeerSetCacheUpdater {
    ...
    pub async fn update(&self) {
        for (chain_id, url) in self.query_addresses.iter() {
            let client = pont_rest_client::Client::new(Url::parse(url).unwrap());
...
mod tests {
    ...
    use pont_infallible::RwLock;
    use pont_rest_client::pont_api_types::*;
    use pont_types::{
    ...
    async fn test_validator_cache_updater_with_valid_address() {
        ...
        let validator_info = ValidatorInfo::new(
            ...
            ValidatorConfig::new(
                ...
                bcs::to_bytes(&vec![NetworkAddress::from_str("/dns/fullnode0.testnet.pontlabs.com/tcp/6182/noise-ik/0xea19ab47ed9191865f15d85d751ed0663205c0b2f0f465714b1947c023715973/handshake/0").unwrap()]).unwrap(),
                2,
            ),
        );
