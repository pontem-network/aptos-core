...
use pont_telemetry_service::types::telemetry::TelemetryEvent;
...
fn collect_mempool_metrics(core_metrics: &mut BTreeMap<String, String>) {
    core_metrics.insert(
        ...
        pont_mempool::counters::CORE_MEMPOOL_INDEX_SIZE
...
fn collect_rest_metrics(core_metrics: &mut BTreeMap<String, String>) {
    let rest_response_metrics = pont_api::metrics::RESPONSE_STATUS.collect();
...
fn collect_storage_metrics(core_metrics: &mut BTreeMap<String, String>) {
    core_metrics.insert(
        ...
        pontdb::metrics::LEDGER_VERSION.get().to_string(),
    );
    core_metrics.insert(
        STORAGE_MIN_READABLE_LEDGER_VERSION.into(),
        pontdb::metrics::PRUNER_LEAST_READABLE_VERSION
