...
use pont_logger::{
    pont_logger::RUST_LOG_TELEMETRY, prelude::*, telemetry_log_writer::TelemetryLog,
    LoggerFilterUpdater,
};
use pont_telemetry_service::types::telemetry::{TelemetryDump, TelemetryEvent};
use pont_types::chain_id::{ChainId, NamedChain};
use rand::Rng;
use rand_core::OsRng;
...
pub fn start_telemetry_service(
    ...
    if telemetry_is_disabled() {
        warn!("Pont telemetry is disabled!");
        return None;
    }
    ...
    let telemetry_runtime = Builder::new_multi_thread()
        ...
        .enable_all()
        .build()
        .expect("Failed to create the Pont Telemetry runtime!");
...
async fn spawn_telemetry_service(
    ...
    if !force_enable_telemetry() && !telemetry_sender.check_chain_access(chain_id).await {
        warn!(
                "Pont telemetry is not sent to the telemetry service because the service is not configured for chain ID {}",
                chain_id
            );
        ...
        loop {
            ...
            if telemetry_sender.check_chain_access(chain_id).await {
                handle.abort();
                info!("Pont telemetry service is now configured for Chain ID {}. Starting telemetry service...", chain_id);
                break;
            }
        }
    }
