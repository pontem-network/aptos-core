...
/// Tool for interacting with configuration of the Pont CLI tool
///
...
impl CliCommand<()> for GenerateShellCompletions {
    ...
    async fn execute(self) -> CliTypedResult<()> {
        ...
        generate(self.shell, &mut command, "pont".to_string(), &mut file);
        Ok(())
    }
}
...
pub struct SetGlobalConfig {
    ...
    ///
    /// `Workspace` will put the `.pont/` folder in the current directory, where
    /// `Global` will put the `.pont/` folder in your home directory
    #[clap(long)]
...
pub enum ConfigType {
    /// Per system user configuration put in `<HOME>/.pont`
    Global,
    /// Per directory configuration put in `<CURRENT_DIR>/.pont`
    Workspace,
}
