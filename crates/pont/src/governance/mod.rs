...
use pont_rest_client::pont_api_types::{Address, HexEncodedBytes, U128, U64};
use pont_rest_client::{Client, Transaction};
use pont_sdk::move_types::language_storage::CORE_CODE_ADDRESS;
use pont_types::event::EventHandle;
use pont_types::governance::VotingRecords;
use pont_types::stake_pool::StakePool;
use pont_types::state_store::table::TableHandle;
use pont_types::{
    ...
    async fn execute(mut self) -> CliTypedResult<Vec<ProposalSummary>> {
        ...
        let events = client
            .get_account_events_bcs(
                AccountAddress::ONE,
                "0x1::pont_governance::GovernanceEvents",
...
async fn get_proposal(
    client: &pont_rest_client::Client,
...
impl CliCommand<ProposalSubmissionSummary> for SubmitProposal {
    ...
    async fn execute(mut self) -> CliTypedResult<ProposalSubmissionSummary> {
        ...
        let txn = self
            .txn_options
            .submit_transaction(pont_stdlib::pont_governance_create_proposal(
        ...
        if let Transaction::UserTransaction(inner) = txn {
            ...
            let proposal_id = if let Some(event) = inner.events.into_iter().find(|event| {
                event.typ.to_string().as_str() == "0x1::pont_governance::CreateProposalEvent"
            }) {
    ...
    async fn execute(mut self) -> CliTypedResult<Vec<TransactionSummary>> {
        ...
        let voting_records = client
            .get_account_resource_bcs::<VotingRecords>(
                CORE_CODE_ADDRESS,
                "0x1::pont_governance::VotingRecords",
            )
            .await
            .unwrap()
            .into_inner()
        ...
        for pool_address in self.pool_addresses {
            ...
            summaries.push(
                self.txn_options
                    .submit_transaction(pont_stdlib::pont_governance_vote(
                        pool_address,
                        proposal_id,
                        vote,
                    ))
...
struct JsonProposal {
    ...
    execution_hash: pont_rest_client::pont_api_types::HashValue,
    expiration_secs: U64,
    is_resolved: bool,
    min_vote_threshold: U128,
    no_votes: U128,
