...
use pont_logger::{debug, Level};
use pont_rest_client::pont_api_types::HashValue;
use pont_rest_client::{Account, Client};
use pont_telemetry::service::telemetry_is_disabled;
use pont_types::{chain_id::ChainId, transaction::authenticator::AuthenticationKey};
use reqwest::Url;
use serde::Serialize;
#[cfg(unix)]
...
async fn send_telemetry_event(
    ...
    // Send the event
    pont_telemetry::cli_metrics::send_cli_telemetry_event(
        build_information,
        command.into(),
        latency,
        success,
