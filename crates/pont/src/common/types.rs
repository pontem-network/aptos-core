...
use pont_global_constants::adjust_gas_headroom;
use pont_keygen::KeyGen;
use pont_rest_client::error::RestError;
use pont_rest_client::pont_api_types::{ExplainVMStatus, HashValue, UserTransaction};
use pont_rest_client::{Client, Transaction};
...
pub enum CliError {
    ...
    #[error("Unable to find config {0}, have you run `pont init`?")]
...
impl From<pont_config::config::Error> for CliError {
    fn from(e: pont_config::config::Error) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}

impl From<pont_github_client::Error> for CliError {
    fn from(e: pont_github_client::Error) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
impl From<pont_crypto::CryptoMaterialError> for CliError {
    fn from(e: pont_crypto::CryptoMaterialError) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
/// Config saved to `.pont/config.yaml`
...
pub const CONFIG_FOLDER: &str = ".pont";
...
pub struct ProfileConfig {
    ...
    /// URL for the Pont rest endpoint
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl CliConfig {
    ...
    pub fn config_exists(mode: ConfigSearchMode) -> bool {
        if let Ok(folder) = Self::pont_folder(mode) {
    ...
    pub fn load(mode: ConfigSearchMode) -> CliTypedResult<Self> {
        let folder = Self::pont_folder(mode)?;
    ...
    /// Saves the config to ./.pont/config.yaml
    pub fn save(&self) -> CliTypedResult<()> {
        let pont_folder = Self::pont_folder(ConfigSearchMode::CurrentDir)?;

        // Create if it doesn't exist
        create_dir_if_not_exist(pont_folder.as_path())?;

        // Save over previous config file
        let config_file = pont_folder.join(CONFIG_FILE);
        ...
        let legacy_config_file = pont_folder.join(LEGACY_CONFIG_FILE);
    ...
    /// Finds the current directory's .pont folder
    fn pont_folder(mode: ConfigSearchMode) -> CliTypedResult<PathBuf> {
...
impl RestOptions {
    ...
    pub fn url(&self, profile: &ProfileOptions) -> CliTypedResult<reqwest::Url> {
        ...
        .map(|p| p.rest_url)
        {
            ...
            Err(CliError::CommandArgumentError("No rest url given.  Please add --url or add a rest_url to the .pont/config.yaml for the current profile".to_string()))
        }
    }
...
pub fn load_account_arg(str: &str) -> Result<AccountAddress, CliError> {
            ...
            .map(|p| p.private_key)
    {
        ...
        Err(CliError::CommandArgumentError(
            "'--account' or '--profile' after using pont init must be provided".to_string(),
        ))
    }
}
...
pub struct FaucetOptions {
    /// URL for the faucet endpoint e.g. https://faucet.devnet.pontlabs.com
    #[clap(long)]
...
impl FaucetOptions {
    ...
    pub fn faucet_url(&self, profile: &ProfileOptions) -> CliTypedResult<reqwest::Url> {
        ...
        .map(|profile| profile.faucet_url)
        {
            ...
            Err(CliError::CommandArgumentError("No faucet given.  Please add --faucet-url or add a faucet URL to the .pont/config.yaml for the current profile".to_string()))
        }
    }
}
