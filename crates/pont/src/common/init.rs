...
use clap::Parser;
use pont_crypto::{ed25519::Ed25519PrivateKey, PrivateKey, ValidCryptoMaterialStringExt};
use pont_rest_client::error::{PontErrorResponse, RestError};
use pont_rest_client::pont_api_types::{PontError, PontErrorCode};
use reqwest::Url;
...
/// Tool to initialize current directory for the pont tool
///
/// Configuration will be pushed into .pont/config.yaml
#[derive(Debug, Parser)]
pub struct InitTool {
...
impl CliCommand<()> for InitTool {
    ...
    async fn execute(self) -> CliTypedResult<()> {
        ...
        let mut profile_config = if let Some(profile_config) = config.remove_profile(profile_name) {
            prompt_yes_with_override(&format!("Pont already initialized for profile {}, do you want to overwrite the existing config?", profile_name), self.prompt_options)?;
            profile_config
        } else {
        ...
        match network {
            Network::Mainnet => {
                profile_config.rest_url = Some("https://fullnode.mainnet.pontlabs.com".to_string());
                profile_config.faucet_url = None;
            }
            Network::Testnet => {
                profile_config.rest_url = Some("https://fullnode.testnet.pontlabs.com".to_string());
                profile_config.faucet_url = None;
            }
            Network::Devnet => {
                profile_config.rest_url = Some("https://fullnode.devnet.pontlabs.com".to_string());
                profile_config.faucet_url = Some("https://faucet.devnet.pontlabs.com".to_string());
            }
            Network::Local => {
        ...
        let client = pont_rest_client::Client::new(
        ...
        let account_exists = match client.get_account(address).await {
            Ok(_) => true,
            Err(err) => {
                if let RestError::Api(PontErrorResponse {
                    error:
                        PontError {
                            error_code: PontErrorCode::ResourceNotFound,
                            ..
                        },
                    ..
                })
                | RestError::Api(PontErrorResponse {
                    error:
                        PontError {
                            error_code: PontErrorCode::AccountNotFound,
                            ..
                        },
                    ..
                }) = err
        ...
        if let Some(ref faucet_url) = profile_config.faucet_url {
            ...
            eprintln!("Account {} does not exist, you will need to create and fun the account through a community faucet e.g. https://pontlabs.com/testnet-faucet, or by transferring funds from another account", address);
        ...
        config.save()?;
        eprintln!("\n---\nPont CLI is now set up for account {} as profile {}!  Run `pont --help` for more information about commands", address, self.profile_options.profile_name().unwrap_or(DEFAULT_PROFILE));
        Ok(())
    }
}
