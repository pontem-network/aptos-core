...
use pont_rest_client::{
    pont_api_types::{WriteResource, WriteSetChange},
    Transaction,
};
...
impl CliCommand<TransferSummary> for TransferCoins {
    ...
    async fn execute(self) -> CliTypedResult<TransferSummary> {
        self.txn_options
            .submit_transaction(pont_stdlib::pont_account_transfer(
                self.account,
                self.amount,
            ))
            .await
...
const SUPPORTED_COINS: [&str; 1] = ["0x1::coin::CoinStore<0x1::pont_coin::PontCoin>"];
