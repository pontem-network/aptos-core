...
use pont_rest_client::VersionedNewBlockEvent;
use pont_types::account_address::AccountAddress;
use pont_types::account_config::{new_block_event_key, NewBlockEvent};
...
impl AnalyzeValidators {
    ...
    pub fn fetch_epoch(epoch: u64, pont_db: &dyn DbReader) -> Result<Vec<VersionedNewBlockEvent>> {
        let batch = 100;

        let mut cursor = u64::max_value();
        let mut result: Vec<VersionedNewBlockEvent> = vec![];
        let ledger_version = pont_db.get_latest_ledger_info()?.ledger_info().version();

        loop {
            let raw_events = pont_db.get_events(
                &new_block_event_key(),
                cursor,
                Order::Descending,
                batch as u64,
