...
pub struct BootstrapDbFromBackup {
    ...
    #[clap(
        long = "target-db-dir",
        help = "Target dir where the tool recreates a PontDB with snapshots and transactions provided \
        in the backup. The data folder can later be used to start an Pont node. e.g. /opt/pont/data/db",
        parse(from_os_str)
    )]
    pub db_dir: PathBuf,
