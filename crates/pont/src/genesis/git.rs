...
use pont_config::config::Token;
use pont_genesis::config::Layout;
use pont_github_client::Client as GithubClient;
...
impl FromStr for GithubRepo {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        if parts.len() != 2 {
            Err(CliError::CommandArgumentError("Invalid repository must be of the form 'owner/repository` e.g. 'aptos-labs/pont-core'".to_string()))
        } else {
            Ok(GithubRepo {
...
pub struct GitOptions {
    /// Github repository e.g. 'aptos-labs/pont-core'
    ///
