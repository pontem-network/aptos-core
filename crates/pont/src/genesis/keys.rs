...
use clap::Parser;
use pont_genesis::config::{Layout, OperatorConfiguration, OwnerConfiguration};
use pont_genesis::keys::PublicIdentity;
use pont_genesis::{config::HostAndPort, keys::generate_key_objects};
use pont_types::account_address::AccountAddress;
use pont_types::transaction::{Script, Transaction, WriteSetPayload};
...
pub struct SetValidatorConfiguration {
    ...
    /// Host and port pair for the validator e.g. 127.0.0.1:6180 or pontlabs.com:6180
    #[clap(long)]
    pub(crate) validator_host: HostAndPort,

    /// Host and port pair for the fullnode e.g. 127.0.0.1:6180 or pontlabs.com:6180
    #[clap(long)]
