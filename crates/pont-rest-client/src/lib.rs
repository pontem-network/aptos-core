...
use crate::pont::{Balance, PontVersion};
...
use pont_api_types::{
    ...
    BcsBlock, Block, Bytecode, ExplainVMStatus, GasEstimation, HexEncodedBytes, IndexResponse,
    MoveModuleId, PontError, TransactionData, TransactionOnChainData,
    TransactionsBatchSubmissionResult, UserTransaction, VersionedEvent,
};
use pont_crypto::HashValue;
use pont_logger::{debug, info, sample, sample::SampleRate};
use pont_types::{
...
pub const USER_AGENT: &str = concat!("pont-client-sdk-rust / ", env!("CARGO_PKG_VERSION"));
pub const DEFAULT_VERSION_PATH_BASE: &str = "v1/";
const DEFAULT_MAX_WAIT_MS: u64 = 300000000000;
const DEFAULT_INTERVAL_MS: u64 = 1000000000;
...
const DEFAULT_MAX_SERVER_LAG_WAIT_DURATION: Duration = Duration::from_secs(30000000);

type PontResult<T> = Result<T, RestError>;
...
impl Client {
    ...
    pub fn version_path_base(mut self, version_path_base: String) -> PontResult<Self> {
    ...
    fn build_path(&self, path: &str) -> PontResult<Url> {
        Ok(self.base_url.join(&self.version_path_base)?.join(path)?)
    }

    pub async fn get_pont_version(&self) -> PontResult<Response<PontVersion>> {
        self.get_resource::<PontVersion>(CORE_CODE_ADDRESS, "0x1::version::Version")
            .await
    }
    ...
    pub async fn get_index(&self) -> PontResult<Response<IndexResponse>> {
        self.get(self.build_path("")?).await
    }

    pub async fn get_index_bcs(&self) -> PontResult<Response<IndexResponseBcs>> {
        let url = self.build_path("")?;
        let response = self.get_bcs(url).await?;
        Ok(response.and_then(|inner| bcs::from_bytes(&inner))?)
    }

    pub async fn get_ledger_information(&self) -> PontResult<Response<State>> {
    ...
    ) -> PontResult<Response<Vec<UserTransaction>>> {
    ...
    pub async fn simulate_bcs(
        &self,
        txn: &SignedTransaction,
    ) -> PontResult<Response<TransactionOnChainData>> {
        let txn_payload = bcs::to_bytes(txn)?;
        let url = self.build_path("transactions/simulate")?;
    ...
    pub async fn submit(
        &self,
        txn: &SignedTransaction,
    ) -> PontResult<Response<PendingTransaction>> {
        let txn_payload = bcs::to_bytes(txn)?;
    ...
    pub async fn submit_bcs(&self, txn: &SignedTransaction) -> PontResult<Response<()>> {
        let txn_payload = bcs::to_bytes(txn)?;
        let url = self.build_path("transactions")?;
    ...
    ) -> PontResult<Response<TransactionsBatchSubmissionResult>> {
    ...
    pub async fn submit_and_wait(
        &self,
        txn: &SignedTransaction,
    ) -> PontResult<Response<Transaction>> {
    ...
    async fn wait_for_transaction_by_hash_inner<F, Fut, T>(
        ...
        fetch: F,
    ) -> PontResult<Response<T>>
    where
        F: Fn(HashValue) -> Fut,
        Fut: Future<Output = PontResult<WaitForTransactionResult<T>>>,
    {
        ...
        loop {
            ...
            match fetch(hash).await? {
                ...
                WaitForTransactionResult::NotFound(error) => {
                    if let RestError::Api(pont_error_response) = error {
                        if let Some(state) = pont_error_response.state {
                            if expiration_timestamp_secs <= state.timestamp_usecs / 1_000_000 {
                                if reached_mempool {
                                    return Err(anyhow!("Transaction expired. It is guaranteed it will not be committed on chain.").into());
                                } else {
            ...
            if let Some(max_server_lag_wait_duration) = max_server_lag_wait {
                if pont_infallible::duration_since_epoch().as_secs()
    ...
    pub async fn get_transactions(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> PontResult<Response<Vec<Transaction>>> {
        let url = self.build_path("transactions")?;
    ...
    pub async fn get_transactions_bcs(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> PontResult<Response<Vec<TransactionOnChainData>>> {
        let url = self.build_path("transactions")?;
    ...
    pub async fn get_transaction_by_hash_bcs(
        &self,
        hash: HashValue,
    ) -> PontResult<Response<TransactionData>> {
    ...
    pub async fn get_account_resources(
        &self,
        address: AccountAddress,
    ) -> PontResult<Response<Vec<Resource>>> {
    ...
    pub async fn get_account_resources_bcs(
        &self,
        address: AccountAddress,
    ) -> PontResult<Response<BTreeMap<StructTag, Vec<u8>>>> {
        let url = self.build_path(&format!("accounts/{}/resources", address))?;
        let response = self.get_bcs(url).await?;
        Ok(response.and_then(|inner| bcs::from_bytes(&inner))?)
    }
    ...
    pub async fn get_account_resource(
        ...
        address: AccountAddress,
        resource_type: &str,
    ) -> PontResult<Response<Option<Resource>>> {
    ...
    pub async fn get_account_modules(
        &self,
        address: AccountAddress,
    ) -> PontResult<Response<Vec<MoveModuleBytecode>>> {
    ...
    pub async fn get_account_modules_bcs(
        &self,
        address: AccountAddress,
    ) -> PontResult<Response<BTreeMap<MoveModuleId, Vec<u8>>>> {
        let url = self.build_path(&format!("accounts/{}/modules", address))?;
        let response = self.get_bcs(url).await?;
        Ok(response.and_then(|inner| bcs::from_bytes(&inner))?)
    }
    ...
    pub async fn get_account_module(
        ...
        address: AccountAddress,
        module_name: &str,
    ) -> PontResult<Response<MoveModuleBytecode>> {
    ...
    pub async fn get_account_module_bcs(
        ...
        address: AccountAddress,
        module_name: &str,
    ) -> PontResult<Response<bytes::Bytes>> {
        let url = self.build_path(&format!("accounts/{}/module/{}", address, module_name))?;
    ...
    pub async fn get_account_events(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> PontResult<Response<Vec<VersionedEvent>>> {
        let url = self.build_path(&format!(
            "accounts/{}/events/{}/{}",
    ...
    pub async fn get_account_events_bcs(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> PontResult<Response<Vec<EventWithVersion>>> {
        let url = self.build_path(&format!(
            "accounts/{}/events/{}/{}",
            address.to_hex_literal(),
            struct_tag,
    ...
    pub async fn get_account(&self, address: AccountAddress) -> PontResult<Response<Account>> {
    ...
    pub async fn get_account_bcs(
        &self,
        address: AccountAddress,
    ) -> PontResult<Response<AccountResource>> {
        let url = self.build_path(&format!("accounts/{}", address))?;
        let response = self.get_bcs(url).await?;
        Ok(response.and_then(|inner| bcs::from_bytes(&inner))?)
    }

    pub async fn estimate_gas_price(&self) -> PontResult<Response<GasEstimation>> {
        let url = self.build_path("estimate_gas_price")?;
        let response = self.inner.get(url).send().await?;
        self.json(response).await
    }

    pub async fn set_failpoint(&self, name: String, actions: String) -> PontResult<String> {
    ...
    ) -> PontResult<(reqwest::Response, State)> {
        if !response.status().is_success() {
            Err(parse_error(response).await)
        } else {
    ...
    pub async fn health_check(&self, seconds: u64) -> PontResult<()> {
    ...
    async fn get<T: DeserializeOwned>(&self, url: Url) -> PontResult<Response<T>> {
        self.json(self.inner.get(url).send().await?).await
    }

    async fn get_bcs(&self, url: Url) -> PontResult<Response<bytes::Bytes>> {
        let response = self.inner.get(url).header(ACCEPT, BCS).send().await?;
    ...
    where
        F: Fn() -> Fut,
        RetryFun: Fn(StatusCode, Option<PontError>) -> bool,
        Fut: Future<Output = PontResult<T>>,
    {
...
pub fn retriable_with_404(status_code: StatusCode, pont_error: Option<PontError>) -> bool {
    retriable(status_code, pont_error) | matches!(status_code, StatusCode::NOT_FOUND)
}

pub fn retriable(status_code: StatusCode, _pont_error: Option<PontError>) -> bool {
    matches!(
        status_code,
...
fn parse_state(response: &reqwest::Response) -> PontResult<State> {
...
async fn parse_error(response: reqwest::Response) -> RestError {
    ...
    match response.json::<PontError>().await {
