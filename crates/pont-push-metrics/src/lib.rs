...
use pont_logger::{error, info, warn};
use pont_metrics_core::{Encoder, TextEncoder};
...
#[deprecated(
    note = "The pont-push-metrics crate is deprecrated. In future prefer to use pont-metrics-core directly (pull) or use pont-telemetry (push)."
)]
impl MetricsPusher {
    fn push(push_metrics_endpoint: &str) {
        let mut buffer = Vec::new();

        if let Err(e) = TextEncoder::new().encode(&pont_metrics_core::gather(), &mut buffer) {
