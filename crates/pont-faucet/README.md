...
Faucet is a service for creating and funding accounts on the Pont Network. It is meant to be used for devnets and testnets. By default, the Faucet takes the provided account, creates a new account, mints a lot of Coin<PontCoin> into that account, and delegates minting capability to that account. That account is then used to provide mint services via the faucet.
...
* Base URL: `http://faucet.testnet.pontlabs.com/`
* Path: `/mint`
* Method: POST
...
The response HTTP body is hex encoded bytes of BCS encoded `Vec<pont_types::transaction::SignedTransaction>`.

Decode Example ([source code generator](https://github.com/aptos-labs/pont-core/tree/main/pont-move/pont-sdk-builder)):
...
``` python
  ...
  for i in range(length):
    txns.push(de.deserialize_any(pont_types.SignedTransaction))
...
```bash
curl -X POST http://faucet.testnet.pontlabs.com/mint\?amount\=1000000\&pub_key\=459c77a38803bd53f3adee52703810e3a74fd7c46952c497e75afb0a7932586d\&return_txns\=true
