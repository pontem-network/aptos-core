...
#[cfg(test)]
mod tests {
    use pont_crypto::{ed25519::Ed25519PublicKey, hash::HashValue};
    use pont_faucet::{routes, Service};
    ...
    use pont_warp_webserver::Response;
    use serde::Serialize;
    use std::{
        collections::HashMap,
    ...
    fn setup(maximum_amount: Option<u64>) -> (AccountStates, Arc<Service>) {
        ...
        let accounts = AccountStates::new(pont_infallible::RwLock::new(HashMap::new()));
        accounts
            .write()
    ...
    async fn handle_get_transaction(
        ...
        last_txn.lock().unwrap().as_ref().map_or_else(
            || Err(warp::reject()),
            |txn| {
                let info = pont_rest_client::pont_api_types::TransactionInfo {
                    version: 0.into(),
                ...
                let serializable_txn: pont_rest_client::pont_api_types::Transaction = (
