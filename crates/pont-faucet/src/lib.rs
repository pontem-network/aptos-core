...
// README: The pont-faucet is deprecated in favor of the tap. Do not add new code
// to this until you've spoken with the Ecosystem Platform team + dport.

//! This crate provides the Faucet service for creating and funding accounts on the Pont Network.
//!
...
//!
//! ```bash
//! cargo run --bin pont-faucet -- -c TESTNET -m <mint-private-key-path> -s http://localhost:8080 -p 8081
//! ```
//!
//! Check help doc for options details:
//!
//! ```bash
//! cargo run -p pont-faucet -- -h
//! ```
...
use pont_rest_client::Client;
use pont_sdk::{
    transaction_builder::{pont_stdlib, TransactionFactory},
    types::{
        account_address::AccountAddress, account_config::pont_test_root_address, chain_id::ChainId,
        LocalAccount,
    },
};
...
/// Pont Testnet utility service for creating test accounts and minting test coins
#[derive(Clone, Debug, Parser)]
#[clap(name = "Pont Faucet", author, version)]
pub struct FaucetArgs {
    ...
    pub port: u16,
    /// Pont fullnode/validator server URL
    #[clap(short = 's', long, default_value = "https://testnet.pontlabs.com/")]
    pub server_url: Url,
    /// Path to the private key for creating test account and minting coins.
    /// To keep Testnet simple, we used one private key for pont root account
    /// To manually generate a keypair, use generate-key:
    /// `cargo run -p generate-keypair -- -o <output_file_path>`
    #[clap(
        short = 'm',
        long,
        default_value = "/opt/pont/etc/mint.key",
        parse(from_os_str)
    )]
...
impl FaucetArgs {
    pub async fn run(self) {
        ...
        let faucet_address: AccountAddress = self
            ...
            .unwrap_or_else(pont_test_root_address);
    ...
    pub fn new(
        ...
        Service {
            ...
            transaction_factory: TransactionFactory::new(chain_id)
                .with_gas_unit_price(std::cmp::max(1, pont_global_constants::GAS_UNIT_PRICE))
        ...
        service
            ...
            .submit_and_wait(&faucet_account.sign_with_transaction_builder(
                service.transaction_factory.payload(
                    pont_stdlib::pont_coin_delegate_mint_capability(delegated_account.address()),
                ),
            ))
            .await
    ...
    service
        ...
        .submit_and_wait(
            &delegated_account.sign_with_transaction_builder(
                service
                    .transaction_factory
                    .payload(pont_stdlib::pont_coin_claim_mint_capability()),
            ),
        )
        .await
        .unwrap();
