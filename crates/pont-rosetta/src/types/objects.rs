...
use pont_crypto::{ed25519::Ed25519PublicKey, ValidCryptoMaterialStringExt};
use pont_logger::warn;
use pont_rest_client::pont_api_types::TransactionOnChainData;
use pont_rest_client::pont_api_types::U64;
use pont_types::account_config::{AccountResource, CoinStoreResource, WithdrawEvent};
use pont_types::contract_event::ContractEvent;
use pont_types::stake_pool::{SetOperatorEvent, StakePool};
use pont_types::state_store::state_key::StateKey;
use pont_types::transaction::{EntryFunction, TransactionPayload};
use pont_types::write_set::{WriteOp, WriteSet};
use pont_types::{account_address::AccountAddress, event::EventKey};
...
/// Representation of a Block for a blockchain.  For pont it is the version
///
...
pub struct Block {
    ...
    pub timestamp: u64,
    /// Transactions associated with the version.  In pont there should only be one transaction
    pub transactions: Vec<Transaction>,
}

/// A combination of a transaction and the block associated.  In Pont, this is just the same
...
///
/// This is known as a write set change within Pont
...
/// Cryptographic signature type used for signing transactions.  Pont only uses
    ...
    /*    if let Some(owner_address) = server_context.pool_address_to_owner.get(&pool_address) {
            if let Ok(stakepool) = bcs::from_bytes::<StakePool>(data) {
                ...
                let add_stake_events = filter_events(
                    ...
                    |event_key, event| {
                        if let Ok(event) = bcs::from_bytes::<pont_types::stake_pool::AddStakeEvent>(
                            event.event_data(),
                        ) {
                            Some(event)
                        } else {
                ...
                let set_operator_events = filter_events(
                    ...
                    |event_key, event| {
                        if let Ok(event) = bcs::from_bytes::<pont_types::stake_pool::SetOperatorEvent>(
                            event.event_data(),
                        ) {
                            Some(event)
                        } else {
...
impl InternalOperation {
    ...
    pub fn payload(
        &self,
    ) -> ApiResult<(pont_types::transaction::TransactionPayload, AccountAddress)> {
        Ok(match self {
            InternalOperation::CreateAccount(create_account) => (
                pont_stdlib::pont_account_create_account(create_account.new_account),
                create_account.sender,
            ),
            InternalOperation::Transfer(transfer) => {
                is_native_coin(&transfer.currency)?;
                (
                    pont_stdlib::pont_account_transfer(transfer.receiver, transfer.amount.0),
                    transfer.sender,
                )
            }
