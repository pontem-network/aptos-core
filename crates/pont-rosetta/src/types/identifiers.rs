...
use pont_types::transaction::TransactionInfo;
...
/// Identifier for a "block".  In pont, we use a transaction model, so the index
...
impl BlockIdentifier {
    pub fn from_block(
        block: &pont_rest_client::pont_api_types::BcsBlock,
        chain_id: ChainId,
    ) -> BlockIdentifier {
        BlockIdentifier {
...
pub struct NetworkIdentifier {
    /// Blockchain name, should always be `pont` and be hardcoded
...
impl From<pont_crypto::HashValue> for TransactionIdentifier {
    fn from(hash: pont_crypto::HashValue) -> Self {
        TransactionIdentifier {
