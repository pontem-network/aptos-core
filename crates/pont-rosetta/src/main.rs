...
use clap::Parser;
use pont_config::config::{ApiConfig, DEFAULT_MAX_PAGE_SIZE};
use pont_logger::prelude::*;
use pont_node::PontNodeArgs;
use pont_rosetta::bootstrap;
...
async fn main() {
    ...
    match args {
        CommandArgs::OnlineRemote(_) => {
            println!("pont-rosetta: Starting Rosetta in Online remote (no local full node) mode")
        }
        CommandArgs::Online(_) => {
            println!("pont-rosetta: Starting Rosetta in Online (with local full node) mode")
        }
        CommandArgs::Offline(_) => println!("pont-rosetta: Starting Rosetta in Offline mode"),
    }
    ...
    }) = args
    {
            ...
            match client.get_index_bcs().await {
                ...
                Err(err) => {
                    sample!(
                        ...
                        println!(
                            "pont-rosetta: Full node REST API isn't responding yet.  You should check the node logs.  It's been waiting {} seconds.  Error: {:?}",
        ...
        println!("pont-rosetta: Local full node started successfully");
        Some(runtime)
    } else {
    ...
    println!("pont-rosetta: Starting rosetta");
    ...
    .expect("pont-rosetta: Should bootstrap rosetta server");

    println!("pont-rosetta: Rosetta started");
...
trait ServerArgs {
    ...
    fn rest_client(&self) -> Option<pont_rest_client::Client>;
...
/// Pont Rosetta API Server
///
/// Provides an implementation of [Rosetta](https://www.rosetta-api.org/docs/Reference.html) on Pont.
#[derive(Debug, Parser)]
#[clap(name = "pont-rosetta", author, version, propagate_version = true)]
...
pub struct OnlineRemoteArgs {
    ...
    /// URL for the Pont REST API. e.g. https://fullnode.devnet.pontlabs.com
...
impl ServerArgs for OnlineRemoteArgs {
    ...
    fn rest_client(&self) -> Option<pont_rest_client::Client> {
        Some(pont_rest_client::Client::new(self.rest_api_url.clone()))
    }
...
impl ServerArgs for OnlineLocalArgs {
    ...
    fn rest_client(&self) -> Option<pont_rest_client::Client> {
        Some(pont_rest_client::Client::new(
