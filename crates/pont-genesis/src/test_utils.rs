...
use pont_config::config::{IdentityBlob, NodeConfig};
...
pub fn test_config() -> (NodeConfig, Ed25519PrivateKey) {
    ...
    let mut test = pont_config::config::TestConfig::new_with_temp_dir(Some(path));

    test.owner_key(account_private_key.unwrap());
    config.test = Some(test);

    let mut sr_test = pont_config::config::SafetyRulesTestConfig::new(account_address.unwrap());
