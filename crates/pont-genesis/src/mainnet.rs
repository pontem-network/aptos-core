...
impl MainnetGenesisInfo {
    ...
    fn generate_genesis_txn(&self) -> Transaction {
        vm_genesis::encode_pont_mainnet_genesis_transaction(
            &self.accounts,
    ...
    pub fn generate_waypoint(&mut self) -> anyhow::Result<Waypoint> {
        ...
        let db_rw = DbReaderWriter::new(pontdb);
        executor::db_bootstrapper::generate_waypoint::<PontVM>(&db_rw, genesis)
    }
}
