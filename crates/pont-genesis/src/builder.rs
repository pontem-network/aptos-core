...
use pont_types::{chain_id::ChainId, transaction::Transaction, waypoint::Waypoint};
use rand::Rng;
...
fn set_identity_for_network(network: &mut NetworkConfig) -> anyhow::Result<()> {
    if let Identity::None = network.identity {
        ...
        let peer_id = pont_types::account_address::from_identity_public_key(key.public_key());
        ...
        R: rand::RngCore + rand::CryptoRng,
    {
        ...
        let fullnode_network_listen_address =
            if let Some(template_fullnode_config) = config.full_node_networks.first() {
                ...
                pont_config::utils::get_available_port_in_multiaddr(true)
            };
        ...
        let vfn_network = NetworkConfig {
            listen_address: pont_config::utils::get_available_port_in_multiaddr(true),
