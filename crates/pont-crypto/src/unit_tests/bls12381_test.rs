...
use crate::{
    ...
    test_utils::{random_subset, KeyPair, TestPontCrypto},
...
fn random_message_for_signing(rng: &mut OsRng) -> TestPontCrypto {
    TestPontCrypto(
...
fn random_messages_for_signing(rng: &mut OsRng, n: usize) -> Vec<TestPontCrypto> {
    (0..n)
        .map(|_| random_message_for_signing(rng))
        .collect::<Vec<TestPontCrypto>>()
}
...
fn bls12381_aggsig_should_verify() {
    ...
    let msgs_refs = messages.iter().collect::<Vec<&TestPontCrypto>>();
    ...
    let msgs_wrong_refs = messages_wrong.iter().collect::<Vec<&TestPontCrypto>>();
...
/// pont-move/framework/move-stdlib/sources/signer.move
...
fn bls12381_sample_signature_verifies() {
    ...
    assert!(sig.verify_arbitrary_msg(b"Hello Pont!", &pk).is_ok());
}
...
fn bls12381_sample_doc_test_for_normal_sigs() {
    ...
    println!("// signature on TestPontCrypto(\"test\".to_owned())");
    println!(
...
fn bls12381_sample_aggregate_pk_and_aggsig() {
    ...
    println!(
        "// The signed messages are \"Hello, Pont <i>!\", where <i> \\in {{1, ..., {}}}",
        num
    );
