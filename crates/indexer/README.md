...
### Installing fullnode
Please follow standard fullnode installation guide on pont.dev (https://pont.dev/nodes/full-node/fullnode-source-code-or-docker)

### Running indexer
```bash
cargo run --bin pont-node --features "indexer"  -- --config <some_path>/fullnode.yaml
```
...
> - Pont tests use the `INDEXER_DATABASE_URL` env var. It needs to be set for the relevant tests to run.
