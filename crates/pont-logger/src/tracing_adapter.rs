...
/// A layer that translates tracing events into pont-logger events.
pub struct TracingToPontDataLayer;
...
impl<S> Layer<S> for TracingToPontDataLayer
where
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    fn on_event(&self, event: &Event, ctx: Context<S>) {
        let metadata = match translate_metadata(event.metadata()) {
            ...
            None => {
                dl::warn!(
                    "[tracing-to-pont-logger] failed to translate event due to unknown level {:?}",
