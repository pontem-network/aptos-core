...
pub fn set_global_logger(logger: Arc<dyn Logger>, console_port: Option<u16>) {
     ...
     * else pont-logger intercepts all tracing::log events
     * In both scenarios *ALL* pont-logger::log events are captured by pont-logger as usual.
     */
    #[cfg(feature = "pont-console")]
    {
    ...
    if None == console_port {
        let _ = tracing::subscriber::set_global_default(
            crate::tracing_adapter::TracingToPontDataLayer
                .with_subscriber(tracing_subscriber::Registry::default()),
        );
    } else {
        error!("console_port was set but has no effect, build with --cfg pont-console");
    }
}
