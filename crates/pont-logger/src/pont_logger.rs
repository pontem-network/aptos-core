...
/// A builder for a `PontData`, configures what, where, and how to write logs.
pub struct PontDataBuilder {
    channel_size: usize,
...
impl PontDataBuilder {
    ...
    fn build_logger(&mut self) -> Arc<PontData> {
    ...
    pub fn build(&mut self) -> Arc<PontData> {
        let logger = self.build_logger();

        let console_port = if cfg!(feature = "pont-console") {
            self.console_port
        } else {
            None
        };
...
impl FilterTuple {
    fn enabled(&self, metadata: &Metadata) -> bool {
        self.local_filter.enabled(metadata)
            ...
            || self.telemetry_filter.enabled(metadata)
    }
}
...
impl PontData {
    pub fn builder() -> PontDataBuilder {
        PontDataBuilder::new()
    }

    #[allow(clippy::new_ret_no_self)]
    pub fn new() -> PontDataBuilder {
        Self::builder()
    }
...
/// or to a `PontData` for external logging.
...
impl LoggerService {
    pub fn run(mut self) {
        let mut tcp_writer = self.address.take().map(TcpWriter::new);
        ...
        for event in &self.receiver {
            match event {
                LoggerServiceEvent::LogEntry(entry) => {
                    ...
                    if let Some(writer) = &mut tcp_writer {
                        if self
                            .facade
                            .filter
                            .read()
...
/// Example:
/// 2020-03-07 05:03:03 INFO [thread_name] common/pont-logger/src/lib.rs:261 Hello { "world": true }
...
pub struct LoggerFilterUpdater {
    logger: Arc<PontData>,
    logger_builder: PontDataBuilder,
}

impl LoggerFilterUpdater {
    pub fn new(logger: Arc<PontData>, logger_builder: PontDataBuilder) -> Self {
        Self {
            logger,
            logger_builder,
        }
...
#[cfg(test)]
mod tests {
    use super::{LogEntry, PontData};
    use crate::{
        debug, error, info,
        logger::Logger,
        pont_logger::{json_format, RUST_LOG_TELEMETRY},
        trace, warn, Event, Key, KeyValue, Level, LoggerFilterUpdater, Metadata, PontDataBuilder,
    ...
    fn basic() {
        ...
        let expected = format!("{{\"level\":\"INFO\",\"source\":{{\"package\":\"pont_logger\",\"file\":\"crates/pont-logger/src/pont_logger.rs:{line_num}\"}},\"thread_name\":\"{thread_name}\",\"hostname\":\"test-host\",\"timestamp\":\"2022-07-24T23:42:29.540278Z\",\"message\":\"This is a log\",\"data\":{{\"bar\":\"foo_bar\",\"category\":\"name\",\"display\":\"12345\",\"foo\":5,\"test\":true}}}}");
    ...
    fn new_async_logger() -> (PontDataBuilder, Arc<PontData>) {
        let mut logger_builder = PontDataBuilder::new();
