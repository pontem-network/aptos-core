## Fuzzing support for Pont

This crate contains support for fuzzing Pont targets. This support sincludes:
...
First, switch to the directory this README is in: `cd testsuite/pont-fuzzer`.

To list out known fuzz targets, run `cargo run --bin pont-fuzzer list`.
...
a corpus with `cargo run --bin pont-fuzzer generate <target>`.

Once a corpus has been generated, the fuzzer is ready to use, simply run:

```
RUSTC_BOOTSTRAP=1 cargo run --bin pont-fuzzer --release fuzz <target>
```

For more options, run `cargo run --bin pont-fuzzer -- --help`. Note that `RUSTC_BOOTSTRAP=1` is
...
Once that has been done, `cargo run --bin pont-fuzzer list` should list your new target.
...
```sh
FUZZ_TARGET=compiled_module cargo flamegraph -p pont-fuzzer --bin flamegraph
```
...
```sh
RUSTFLAGS='--cfg feature="fuzzing"' CORPUS_PATH=fuzz/corpus cargo xtest --html-cov-dir <some path for html output> -p pont-fuzzer -- --test-threads 1 --ignored coverage
```
...
```sh
cd pont-core/testsuite/pont-fuzzer
...
the following to the end of `pont-core/Cargo.toml`:
