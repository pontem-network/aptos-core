...
use pont_rest_client::{Client as RestClient, PendingTransaction, State, Transaction};
use pont_sdk::{
...
impl<'t> PontContext<'t> {
    ...
    pub fn transaction_factory(&self) -> TransactionFactory {
        let unit_price = std::cmp::max(pont_global_constants::GAS_UNIT_PRICE, 1);
        TransactionFactory::new(self.chain_id()).with_gas_unit_price(unit_price)
    }

    pub fn pont_transaction_factory(&self) -> TransactionFactory {
...
impl<'t> PontPublicInfo<'t> {
        ...
        let create_account_txn =
            self.root_account
                .sign_with_transaction_builder(self.transaction_factory().payload(
                    pont_stdlib::pont_account_create_account(auth_key.derived_address()),
                ));
        self.rest_client
    ...
    pub async fn mint(&mut self, addr: AccountAddress, amount: u64) -> Result<()> {
        let mint_txn = self.root_account.sign_with_transaction_builder(
            self.transaction_factory()
                .payload(pont_stdlib::pont_coin_mint(addr, amount)),
        );
    ...
    pub async fn transfer_non_blocking(
        ...
        let tx = from_account.sign_with_transaction_builder(self.transaction_factory().payload(
            pont_stdlib::pont_coin_transfer(to_account.address(), amount),
        ));
...
pub async fn reconfig(
    ...
    let pont_version = client.get_pont_version().await.unwrap();
    let (current, state) = pont_version.into_parts();
    ...
    info!(
        "Changed pont version from {} (epoch={}, ledger_v={}), to {}, (epoch={}, ledger_v={})",
        current_version,
        state.epoch,
        state.version,
        current_version + 1,
