...
use pont_protos::block_output::v1::BlockOutput;
use prost::Message;
use std::fmt::Debug;
...
fn handle_block(
    ...
    pont_logger::trace!("[{}] inserting block {}", substream_name, block_height);
    ...
    async fn process_substream(
        ...
        let block_output: BlockOutput = match output.data.as_ref().unwrap() {
            ModuleOutputData::MapOutput(data) => {
                pont_logger::debug!(block_height = block_height, "Parsing mapper for block");
