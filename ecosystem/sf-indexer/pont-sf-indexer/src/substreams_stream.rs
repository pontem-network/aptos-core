...
fn stream_blocks(
    ...
    try_stream! {
        loop {
            ...
            match result {
                Ok(stream) => {
                    pont_logger::info!("Blockstreams connected");
                    ...
                    for await response in stream{
                        match process_substreams_response(
                            ...
                            Err(err) => {
                                pont_logger::error!("Received error {:#}", err);
                    ...
                    if !expected_stream_end {
                        pont_logger::error!("Stream blocks complete unexpectedly, expecting stream to always stream blocks");
                    } else {
                        return
                    }
                },
                Err(e) => {
                    ...
                    pont_logger::error!("Unable to connect to endpoint: {:#}", e);
                }
            }
...
async fn process_substreams_response(
    ...
    match response.message {
        ...
        None => {
            pont_logger::info!("Got None on substream message");
            Ok(None)
        }
        _ => Ok(None),
    }
