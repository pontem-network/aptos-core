...
> Indexes Pont Blockchain via Substreams connected to Pont Firehose (https://github.com/streamingfast/firehose-pont). 
...
8. Start indexer
```bash
DATABASE_URL=postgres://postgres@localhost:5432/postgres  cargo run -- --endpoint-url http://localhost:18015 --package-file ../pont-substreams/pont-substreams-v0.0.1.spkg --module-name block_to_block_output
```
...
```bash
alias testnet="cd ~/Desktop/pont-core; CARGO_NET_GIT_FETCH_WITH_CLI=true cargo run -p pont-node -- --test"
```
